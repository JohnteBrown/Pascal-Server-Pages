...
begin
  ...
  CheckEquals('{'#13#10#9'"hello" : "world"'#13#10'}',
    json.ToBeautifiedString(0, 1));

  with json.AddArray('items') do
  begin
    AddValue;
  ...
  CheckEquals('{"hello":"world","items":[null,12.3,true,false,null]}',
    json.ToUnicodeString);
  CheckEquals('{'#13#10 + #9'"hello" : "world",'#13#10 + #9'"items" : ['#13#10 +
    #9#9'null,'#13#10 + #9#9'12.3,'#13#10 + #9#9'true,'#13#10 +
    #9#9'false,'#13#10 + #9#9'null'#13#10 + #9']'#13#10 + '}',
    json.ToBeautifiedString(0, 1));
...
begin
  ...
  try
    ...
    CheckEquals('"templates"', json[0]['servlet'][0]['init-param']
      ['templatePath'].ToUnicodeString, 'json.txt');
...
const
  jsonData =
    '{"Result":[{"Links":[{"UrlTo":"http://atomos.com/ninja/","Anchor":"Manufacturer info",'
...
begin
  ...
  try
    ...
    CheckEquals
      ('http://theeditman.com/blogg/ct.ashx?id=13592790-3605-42fd-9308-73c79199d1eb&url=http%3A%2F%2Fatomos.com%2Fsamurai%2F',
...
{$IFDEF FPC}
  CheckEquals(UTF8Encode(WideChar($44F) + WideChar($AA)),
    TdwsJSONImmediate(json).AsString, 'unicode');
{$ELSE}
  CheckEquals(WideChar($44F) + WideChar($AA), TdwsJSONImmediate(json).AsString,
    'unicode');
{$ENDIF}
  json.Free;
...
begin
  ...
  json2 := TdwsJSONValue.ParseString('{'#13#10 + #9'"Value": ['#13#10 +
    #9']'#13#10 + '}');
...
begin
  ...
  Check(json['missing'][2].ValueType = jvtUndefined,
    'check missing[2] value type');
...
begin
  ...
  CheckEquals('"\t\n\r\b\f\\"', json['test'].ToUnicodeString,
    'specials toString');

  json['test'].Value.AsString := #25#0'ok';
  CheckEquals('"\u0019\u0000ok"', json['test'].ToUnicodeString,
    'very specials');
...
begin
  ...
  CheckException(JSONInvalidNameValueSeparator, EdwsJSONParseError,
    'separator');
  CheckException(JSONInvalidNameValueEnding, EdwsJSONParseError, 'ending');

  CheckException(JSONMissingElementValue, EdwsJSONParseError,
...
const
  cAlternatives: array [1 .. 8] of String = ('1', 'true', 'null', '"a"',
    '{"b":2}', '[1,2]', '{"b":[1,2]}', '[{"b":1}]');
var
  i, j: Integer;
  json: TdwsJSONValue;
begin
  for i := Low(cAlternatives) to High(cAlternatives) do
  begin
    for j := Low(cAlternatives) to High(cAlternatives) do
    begin
      json := TdwsJSONValue.ParseString('{"a":' + cAlternatives[i] + ',"a":' +
        cAlternatives[j] + '}', jdoOverwrite);
      try
...
begin
  ...
  CheckEquals('{"hello":[1,2],"world":{}}', json.ToUnicodeString,
    'replace 1 with [1,2]');

  json.Items['world'] := TdwsJSONImmediate.FromVariant(3);

  CheckEquals('{"hello":[1,2],"world":3}', json.ToUnicodeString,
    'replace {} with 3');
...
begin
  ...
  try
    ...
    jsonArray.Add(TdwsJSONValue.ParseString('false'));
    jsonArray.Add(TdwsJSONValue.ParseString('false'));
    CheckEquals('[true,false,null,"test",{"foo":"bar"},true,false,false,false]',
      jsonArray.ToUnicodeString, '1st');
    jsonArray.Clear;
    jsonArray.Add(TdwsJSONValue.ParseString('true'));
    jsonArray.Add(TdwsJSONValue.ParseString('false'));
    jsonArray.Add(TdwsJSONValue.ParseString('null'));
    ...
    jsonArray.Add(TdwsJSONValue.ParseString('true'));
    jsonArray.Add(TdwsJSONValue.ParseString('false'));
    CheckEquals('[true,false,null,"test",{"foo":"bar"},true,false,true,false]',
      jsonArray.ToUnicodeString, '2nd');
  finally
    jsonArray.Free;
  end;
end;
...
var
  a: TdwsJSONValue;
begin
  a := TdwsJSONObject.ParseString
    ('["hello", "world", "some", "alpha", "stuff"]');
  try
...
begin
  ...
  try
    ...
    CheckPath('.books.1.price', '[10]');

    CheckPath('.books[1].price', '[10]');

    CheckPath('.books[-1]..name', '["Douglas Crockford"]');

    CheckPath('.books[*].price', '[17.96,10,15.67]');

    CheckPath('.books[].id', '[1,2,3]');

    CheckPath('.books..title',
      '["Clean Code","Maintainable JavaScript","JavaScript: The Good Parts"]');

    CheckPath('..leaf', '[[11,22]]');
    CheckPath('..leaf[1]', '[22]');
    CheckPath('..leaf.."0"', '[11]');
    CheckPath('.leaf..nope', '[]');

    CheckPath('.nums[1]', '["one"]');
    CheckPath('.nums.2', '["two"]');
    CheckPath('.nums."3"', '[3]');

    CheckPath(' .books [ 2 ] .title ', '["JavaScript: The Good Parts"]');
  finally
    js.Free;
  end;
end;
...
begin
  var
  j := TdwsJSONValue.ParseString('[-Infinity,NaN,Infinity]');
  try
...
begin
  var
  j := TdwsJSONValue.ParseString('"יא"');
  try
    var
