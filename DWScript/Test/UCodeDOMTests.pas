...
type
  ...
  private
    ...
    function ToOutline(const code: String;
      mode: TOutlineMode = omCompact): String;
...
begin
  ...
  FParser := TdwsParser.Create(FTokRules.CreateTokenizer(nil, nil),
  ...
  begin
    if StrEndsWith(FTests[i], 'conditionals_ifndef.pas') then
      FTests.Delete(i);
    if StrEndsWith(FTests[i], 'include_expr.pas') then
      FTests.Delete(i);
    if StrEndsWith(FTests[i], 'Sections.pas') then
      FTests.Delete(i);
    if StrEndsWith(FTests[i], 'UnfinishedDirective.pas') then
      FTests.Delete(i);
  end;
end;
...
// ToOutline
//
function TCodeDOMTests.ToOutline(const code: String;
  mode: TOutlineMode = omCompact): String;
...
begin
  ...
  try
    ...
    if (mode = omCompact) and (Result <> '') then
    begin
      var
...
begin
  for var i := 0 to FDOMTests.Count - 1 do
  begin
    var
    code := LoadTextFromFile(FDOMTests[i]);
    var
    expected := LoadTextFromFile(ChangeFileExt(FDOMTests[i], '.txt'));
    expected := TrimRight(StringReplace(expected, #13#10, #10, [rfReplaceAll]));
    var
    actual := ToOutline(code, omVerbose);
...
begin
  ...
  begin
    var
    code := LoadTextFromFile(FTests[i]);
    var
    outline := ToOutline(code, omErrorsOnly);
...
begin
  CheckEquals('Main,1Comment,2Token comment <</* a>> [LF]', ToOutline('/* a'));
  CheckEquals
...
begin
  CheckEquals('Main,1Try,2Token try,2TryFinally,3Token finally,2Token end',
