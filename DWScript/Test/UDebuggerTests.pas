...
type
  ...
  private
    ...
    function ReportBreakpointables(breakpointables
      : TdwsBreakpointableLines): String;
...
// DoCreateExternal
//
procedure TDebuggerTests.DoCreateExternal(Info: TProgramInfo;
...
begin
  ...
  if FDebugSuspendEvalExpr <> '' then
    FDebugSuspendLastEvalResult := FDebugger.EvaluateAsString
...
// DoNeedUnit
//
function TDebuggerTests.DoNeedUnit(const unitName: String;
  var unitSource: String): IdwsUnit;
begin
  if unitName = 'MyUnit' then
    unitSource :=
      'unit MyUnit; interface uses MyType; procedure Test; implementation'#10 +
      'procedure Test; begin'#10 + '  PrintLn(myEnumValue);'#10 + 'end;'
...
//
function TDebuggerTests.ReportBreakpointables(breakpointables
  : TdwsBreakpointableLines): String;
var
  i, j: Integer;
  lines: TBits;
...
begin
  ...
  try
    ...
    try
      ...
      try
        CheckEquals(110, expr.Expression.EvalAsInteger(exec.ExecutionObject),
          'i+i*10');
      finally
        expr := nil;
      end;
      ...
      try
        CheckEquals(123, expr.Expression.EvalAsInteger(exec.ExecutionObject),
          'StrToInt(''123'')');
      finally
        expr := nil;
      end;
      ...
      try
        ...
        CheckEquals('Syntax Error: Expression expected [line: 1, column: 4]',
          buf, 'i +* i');
      finally
        expr := nil;
      end;
...
begin
  ...
  try
    CheckEquals(123, expr.Expression.EvalAsInteger
      (expr.Execution.ExecutionObject), 'StrToInt(''113'')+10');
  finally
    expr := nil;
  end;
end;
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('var i := 1;'#13#10 + 'procedure Test;'#13#10 +
    'var i := 2;'#13#10 + 'begin'#13#10 + 'PrintLn(i);'#13#10 // line 5
    + 'end;'#13#10 + 'Test;'); // line 7
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('for var i := 1 to 1 do'#13#10 + 'PrintLn(i);');
  try
    exec := prog.CreateNewExecution;
    try
      FDebugEvalExpr := 'i';
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('begin'#13#10 + 'var i := 123;'#13#10 +
    'Print(i);'#13#10 + 'end');
  try
    exec := prog.CreateNewExecution;
    try
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('for var i := 3 to 3 do'#10 + '  PrintLn(i);'#10);
  try
    exec := prog.CreateNewExecution;
    try
      FDebugEvalExpr := 'i';
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('var s := "z";'#10 + 'for var char in s do'#10 +
    '  PrintLn(char);'#10);
  try
    exec := prog.CreateNewExecution;
    try
...
begin
  FCompiler.Config.CompilerOptions := FCompiler.Config.CompilerOptions +
    [coOptimize];
  prog := FCompiler.Compile('var a := 0;'#13#10 + 'var b := 0;'#13#10 +
    'repeat'#13#10 + '   begin'#13#10 + '      var x :=0;'#13#10 +
    '   end'#13#10 + 'until  (a=b) ;');
  try
    exec := prog.CreateNewExecution;
    try
...
begin
  prog := FCompiler.Compile
    ('var a : array of String; for var i := 1 to 3 do a.Add("*"+i.ToString);'#13#10
    + 'PrintLn(a.Join(","));');
  try
    FDebugEvalAtLine := 2;
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('uses MyUnit;'#10 + 'Test;');
  try
...
begin
  prog := FCompiler.Compile
    ('type EMy = class(Exception); try raise EMy.Create("hello"); except'#10 +
    'on E : Exception do'#10 + 'PrintLn(E.Message);'#10 + 'end;');
  try
    CheckEquals('', prog.Msgs.AsInfo);
...
begin
  prog := FCompiler.Compile('var i := 1;'#13#10 + 'procedure Test;'#13#10 +
    'var i := 2;'#13#10 + 'begin'#13#10 + 'PrintLn(i);'#13#10 + 'end;'#13#10
    + 'Test;');
  CheckEquals('2'#13#10, prog.Execute.Result.ToString, 'Result 1');

  breakpointables := TdwsBreakpointableLines.Create(prog);
  CheckEquals('*MainModule*: 1,3,5,7,', ReportBreakpointables(breakpointables),
    'Case 1');
  breakpointables.Free;

  prog := FCompiler.Compile('var i := 1;'#13#10 + 'procedure Test;'#13#10 +
    'begin'#13#10 + 'var i := 2;'#13#10 + 'PrintLn(i);'#13#10 + 'end;'#13#10 +
    'i:=i+1;');
  CheckEquals('', prog.Execute.Result.ToString, 'Result 2');

  breakpointables := TdwsBreakpointableLines.Create(prog);
  CheckEquals('*MainModule*: 1,4,5,7,', ReportBreakpointables(breakpointables),
    'Case 2');
  breakpointables.Free;
end;
...
begin
  prog := FCompiler.Compile('var i := 2'#13#10 + '+Round(Now);'#13#10 +
    'while False do'#13#10 + 'i += i '#13#10 + ' + Sqr(i);'#13#10 +
    'if True then'#13#10 + 'i += 1;');
  CheckEquals('', prog.Execute.Result.ToString, '3');

  breakpointables := TdwsBreakpointableLines.Create(prog);
  try
    if coOptimize in FCompiler.Config.CompilerOptions then
      CheckEquals('*MainModule*: 1,7,', ReportBreakpointables(breakpointables),
        'optimized')
    else
      CheckEquals('*MainModule*: 1,3,4,6,7,',
        ReportBreakpointables(breakpointables), 'unoptimized');
  finally
    breakpointables.Free;
  end;
end;
    ...
    begin
      ...
      try
        ...
        try
          ...
          CheckTrue(report.Entries[0].Runnable >= 3, 'at least 3 runnables in '
            + algos[i]);
          CheckTrue(report.Entries[0].NonCovered < report.Entries[0].Runnable,
            'non covered below runnables ' + algos[i]);
        finally
          report.Free;
        end;
      finally
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('var i : Integer;'#13#10 + 'procedure Test;'#13#10 +
    'begin'#13#10 + 'Print(Inc(i));'#13#10 + 'end;'#13#10 + 'Test;');
  ...
  try
    ...
    CheckEquals(DBG_NotDebugging, FDebugger.EvaluateAsString('i'),
      'eval after detach');
...
begin
  RunExceptNotification
    ('var i : Integer;'#13#10'raise Exception.Create("here");');
  ...
  RunExceptNotification
    ('var i : Integer;'#13#10'try i := i div i; except end;');
  CheckEquals(' [line: 2, column: 12]', FDebugLastNotificationPos.AsInfo, '3');

  RunExceptNotification
    ('procedure Test;'#13#10'begin'#13#10'var i:=0;'#13#10'i := i div i;'#13#10'end;'#13#10'Test');
...
begin
  ...
  try
    prog := FCompiler.Compile('var a := "a";'#13#10 + 'a += "b";'#13#10 +
      'a += "c";'#13#10);
    FDebugEvalExpr := 'a';
    FDebugEvalAtLine := 3;
...
begin
  ...
  try
    prog := FCompiler.Compile('var a := "a";'#13#10 +
      'ToggleBreakpointEnabled(0);'#13#10 + 'a += "b";'#13#10 + 'Print(a);');
    CheckEquals('', prog.Msgs.AsInfo);
...
begin
  prog := FCompiler.Compile('type TTest = (one, two);'#13#10 +
    'var a := 1;'#13#10 + 'var b := TTest(a);'#13#10);
  CheckEquals('', prog.Msgs.AsInfo);
  ...
  try
    ...
    CheckEquals(5, bits.Size, '5 lines');
    // 3 actual lines + one empty + line zero
...
begin
  ...
  try
    prog := FCompiler.Compile
      ('function Hello : Integer; begin Result := -123; end;'#13#10 +
...
begin
  ...
  try
    prog := FCompiler.Compile('function Hello(j : Integer) : Integer;'#10 +
      'begin'#10 + #9'Result := j;'#10 + 'end;'#10 +
      'for var i := 0 to 10 do'#10 + #9'Hello(i);');
    CheckEquals('', prog.Msgs.AsInfo);
    ...
    (FDebugger.Breakpoints[0] as TdwsDebuggerBreakpointConditional).Condition
      := 'i = 3';
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('function Hello : Integer;'#13#10 + 'begin'#13#10 +
    #9'Result := 34;'#13#10 + 'end;'#13#10 + 'Print(12);'#13#10 +
    'Print(Hello);');
  CheckEquals('', prog.Msgs.AsInfo);
  ...
  try
    ...
    CheckEquals
      (', [line: 5, column: 1],  [line: 6, column: 1],  [line: 3, column: 9], ',
      FStepTest);
