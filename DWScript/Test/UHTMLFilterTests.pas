...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'HTMLFilterScripts' + PathDelim,
    '*.dws', FTests);
...
begin
  ...
  try
    locFilter.PatternEval := '==';
    CheckEquals('Print(1);Print(''2'');Print(3);Print(''4'');',
      locFilter.Process('<?pas==1?>2<?pas==3?>4', nil));
  finally
    locFilter.Free;
  end;
end;
...
begin
  prog := FCompiler.Compile
    ('a<?pas'#10'{$F "B"}'#10'{$F "C"}'#10'{$F "D"}'#10'?>f<?pas= "g" ?>');
  exec := prog.Execute;
  CheckEquals('aB({C} )();'#9'D'#10'fg', exec.Result.ToString,
    'include filtered 1');
end;

// DoInclude
//
procedure THTMLFilterTests.DoInclude(const scriptName: String;
...
begin
  ...
  try
    ...
    CheckEquals('     N'#13#10'  '#13#10'     M  ',
      FFilter.Process('<?pasN'#13#10'?>'#13#10'<?pasM?>', nil), '3');

    CheckEquals('     (1);' + StringOfChar(' ', 1204 + 5) + '(2);',
      TrimRight(FFilter.Process('<?pas=1?>' + StringOfChar(' ',
      1204) + '<?pas=2?>', nil)), 'many spaces');
    CheckEquals('     (1);' + StringOfChar(' ', 1204) + #13#10'     (2);',
      TrimRight(FFilter.Process('<?pas=1?>' + StringOfChar(' ',
      1204) + #13#10'<?pas=2?>', nil)), 'many spaces');
  finally
