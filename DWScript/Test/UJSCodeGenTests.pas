...
const
  cCompilerOptions = cDefaultCompilerOptions + [coSymbolDictionary,
    coContextMap, coAllowClosures];
...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'Algorithms' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'SimpleScripts' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'ArrayPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'AssociativePass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'BuildScripts' + PathDelim,
    dwsFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'InterfacesPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'InnerClassesPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'OverloadsPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'HelpersPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'LambdaPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'PropertyExpressionsPass' +
    PathDelim, pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'SetOfPass' + PathDelim,
    pasFilter, FTests);

  CollectFiles(ExtractFilePath(ParamStr(0)) + 'FunctionsMath' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'FunctionsString' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'FunctionsTime' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'FunctionsVariant' + PathDelim,
    pasFilter, FTests); // *)
  // CollectFiles(ExtractFilePath(ParamStr(0))+'FunctionsRTTI'+PathDelim, pasFilter, FTests);

{$IFDEF JS_BIGINTEGER}
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'BigInteger' + PathDelim,
    pasFilter, FTests);
{$ENDIF}
...
// DoInclude
//
procedure TJSCodeGenTests.DoInclude(const scriptName: string;
...
// DoNeedUnit
//
function TJSCodeGenTests.DoNeedUnit(const unitName: String;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        CheckEquals(GetExpectedResult(FTests[i]), 'Errors >>>>'#13#10 +
          prog.Msgs.AsInfo, FTests[i]);
        Inc(ignored);
        continue;
      end;
...
begin
  ...
  try
    ...
    cmd := 'node "' + jsFileName + '"';
    handle := CreateProcess(nil, PChar(cmd), nil, nil, True, 0, nil, nil,
...
begin
  for i := 1 to 100 do
    if not FChromium.Initialized then
    begin
  ...
  try
    ...
    begin
      ...
      if (Pos('Algorithms', FTests[i]) > 1) or (Pos('Functions', FTests[i]) > 1)
      then
      ...
      try
        ...
        jscode := '(function () {'#13#10 + 'var $testResult = [];'#13#10 +
          ...
          'try {'#13#10 + #13#10 + FCodeGen.CompiledOutput(prog) + #13#10 +
          '} catch(e) {$testResult.splice(0,0,"Errors >>>>\r\nRuntime Error: "+((e.ClassType)?e.FMessage:e.message)+"\r\nResult >>>>\r\n")};'#13#10
          + 'console.log($testResult.join(""));'#13#10 +
          'console.log("HF7dGrJxgqfa7VPz");'#13#10
        // ensure something is sent to the console
          + '})();';
        ...
        var
        p := Pos('HF7dGrJxgqfa7VPz', output);
        if p > 0 then
          SetLength(output, p - 1);
        if prog.Msgs.Count > 0 then
        begin
          output := 'Errors >>>>'#13#10 + prog.Msgs.AsInfo +
            'Result >>>>'#13#10 + output;
        end;
        ...
        begin
          ...
          diagnostic.Add(FTests[i] + ': expected <' + expectedResult +
...
begin
  FCompiler.Config.CompilerOptions := cCompilerOptions +
    [coSymbolDictionary, coContextMap, coVariablesAsVarOnly];
  FCodeGen.Options := FCodeGen.Options + [cgoNoInlineMagics];
  Compilation;
end;
...
begin
  FCompiler.Config.CompilerOptions := cCompilerOptions - [coOptimize] +
...
begin
  FCompiler.Config.CompilerOptions := cCompilerOptions +
    [coOptimize, coVariablesAsVarOnly, coSymbolDictionary];
  FCodeGen.Options := FCodeGen.Options + [cgoNoInlineMagics];
  Execution;
end;
...
begin
  FCompiler.Config.CompilerOptions := cCompilerOptions +
    [coOptimize, coVariablesAsVarOnly];
...
begin
  FCompiler.Config.CompilerOptions := cCompilerOptions +
    [coOptimize, coVariablesAsVarOnly, coSymbolDictionary, coContextMap];
