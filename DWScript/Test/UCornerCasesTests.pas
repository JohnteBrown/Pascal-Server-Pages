...
type
  ...
  public
    ...
    function DoOnNeedUnit(const unitName: String;
...
// Process
//
function TTestFilter.Process(const aText: String;
...
begin
  ...
  begin
    ...
    Result := inherited Process(aText, aMsgs);
end;
...
begin
  ...
  CheckEquals
    ('Syntax Error: Hexadecimal digit expected (found #25) [line: 1, column: 10]'#13#10,
    prog.Msgs.AsInfo);
end;
...
begin
  ...
  for i := 1 to 3 do
    threads[i].Start;
  while (threads[1].FTimeStamp = 0) or (threads[2].FTimeStamp = 0) or
    (threads[3].FTimeStamp = 0) do
    Sleep(10);
...
// DoOnInclude
//
procedure TCornerCasesTests.DoOnInclude(const scriptName: String;
...
// DoOnNeedUnit
//
function TCornerCasesTests.DoOnNeedUnit(const unitName: String;
  var unitSource, unitLocation: String): IdwsUnit;
begin
  if SameText(unitName, 'HelloWorld') then
  begin
...
// DoOnResource
//
procedure TCornerCasesTests.DoOnResource(compiler: TdwsCompiler;
  const resName: String);
begin
  FLastResource := resName;
  if resName = 'missing' then
    compiler.Msgs.AddCompilerError(compiler.Tokenizer.HotPos,
      'Missing resource')
...
begin
  ...
  CheckEquals
    ('Syntax Error: Name of include file expected [line: 1, column: 10]'#13#10,
    prog.Msgs.AsInfo, 'include missing');

  prog := FCompiler.Compile('{$include ''test.dummy''}');

  CheckEquals
    ('Compile Error: Could not find file "test.dummy" on input paths [line: 1, column: 11]'#13#10,
  ...
  exec := prog.Execute;
  CheckEquals('hello world', exec.Result.ToString,
    'exec include via event followup');
end;
...
begin
  ...
  CheckEquals(MSG_MainModule, prog.SourceList[0].NameReference,
    'source list 0');
  ...
  exec := prog.Execute;
  CheckEquals('world happy', exec.Result.ToString,
    'exec include via file followup');
...
begin
  ...
  CheckEquals('Syntax Error: Unexpected end of file (unfinished comment) ' +
    '[line: 2, column: 1, file: comment.inc]'#13#10, prog.Msgs.AsInfo);
end;
...
begin
  ...
  CheckEquals('Syntax Error: End of string constant not found (end of file) ' +
    '[line: 2, column: 1, file: string.inc]'#13#10, prog.Msgs.AsInfo);
end;
...
const
  cCode: String = 'unit Hello;'#13#10 + '//1'#13#10 + 'interface'#13#10 +
    '//2'#13#10 + '{$ifdef TEST}procedure World;{$endif}'#13#10 + '//3'#13#10 +
    'implementation'#13#10 + '//4'#13#10 +
    '{$ifdef TEST}procedure World; begin end;{$endif}'#13#10 + '//5'#13#10;
var
  prog: IdwsProgram;
...
begin
  ...
  try
    ...
    Check(prog.SymbolDictionary.FindSymbolUsage('World', suDeclaration) = nil,
      'Check default');
    ...
    Check(prog.SymbolDictionary.FindSymbolUsage('World', suDeclaration) <> nil,
      'Check 1');
    CheckEquals(5, prog.SymbolDictionary.FindSymbolUsage('World', suDeclaration)
      .ScriptPos.Line, 'Line 1');
    ...
    Check(prog.SymbolDictionary.FindSymbolUsage('World', suDeclaration) <> nil,
      'Check 3');
    CheckEquals(9, prog.SymbolDictionary.FindSymbolUsage('World', suDeclaration)
      .ScriptPos.Line, 'Line 3');
...
begin
  ...
  try
    ...
    CheckNotEquals('', fs.ValidateFileName('c:\www'), 'c:\www');
    CheckNotEquals('', fs.ValidateFileName('C:\wWw\world.txt'),
      'C:\wWw\world.txt');
    CheckNotEquals('', fs.ValidateFileName('c:\www\abc\xyz\ghi.txt'),
      'ghi.txt 1');
    ...
    CheckNotEquals('', fs.ValidateFileName('d:\foo\bar\abc\def\ghi.txt'),
      'ghi.txt 2');
    CheckEquals('', fs.ValidateFileName('c:\foo\bar\abc\def\ghi.txt'),
      'ghi.txt 3');
  finally
    r.Free;
  end;
...
begin
  ...
  exec := prog.Execute;
  CheckEquals
    ('Runtime Error: Maximal recursion exceeded (5 calls) [line: 1, column: 24]'#13#10
    + 'Dummy [line: 1, column: 24]'#13#10 + 'Dummy [line: 1, column: 24]'#13#10
    + 'Dummy [line: 1, column: 24]'#13#10 + ' [line: 1, column: 36]'#13#10,
...
begin
  ...
  exec := prog.Execute;
  CheckEquals
    ('Runtime Error: Maximal data size exceeded (2 Variants) [line: 1, column: 41]'#13#10
    + 'Dummy [line: 1, column: 41]'#13#10 + ' [line: 1, column: 53]'#13#10,
...
begin
  ...
  try
    ...
    prog := FCompiler.Compile('type TObj = Class'#13#10 +
      ' Procedure Proc;'#13#10 + ' Begin'#13#10 + '  var p := @Proc;'#13#10 +
      '  p;'#13#10 + ' End;'#13#10 + 'End;'#13#10 + 'TObj.Create.Proc;'#13#10);
    CheckEquals('', prog.Msgs.AsInfo, 'compile');
    prog.ProgramObject.MaxRecursionDepth := 128;
    exec := prog.Execute;
    buf := 'TObj.Proc [line: 5, column: 4]'#13#10' [line: 8, column: 13]'#13#10;
    buf2 := exec.Msgs.AsInfo;
    CheckEquals(buf, Copy(buf2, Length(buf2) - Length(buf) + 1, MaxInt),
      'stack overflow');
  finally
...
procedure TCornerCasesTests.Assertions;
  ...
  procedure CheckCase(options: TCompilerOptions;
    const expected, testName: String);
  var
    prog: IdwsProgram;
    exec: IdwsProgramExecution;
  begin
...
begin
  try
    CheckCase([coOptimize, coAssertions],
      'Runtime Error: Assertion failed [line: 1, column: 1]',
      'assertions optimization');
    CheckCase([coAssertions],
      'Runtime Error: Assertion failed [line: 1, column: 1]', 'assertions');
...
begin
  prog := FCompiler.Compile('PrintLn(ParamCount);' + 'var i : Integer;' +
    'if ParamCount>0 then Print(Param(0));' +
    'for i:=0 to ParamCount-1 do PrintLn(ParamStr(i));');

  CheckEquals('1'#13#10'hello worldhello world'#13#10,
    prog.ExecuteParam('hello world').Result.ToString);
  CheckEquals('2'#13#10'hellohello'#13#10'world'#13#10,
    prog.ExecuteParam(VarArrayOf(['hello', 'world'])).Result.ToString);

  SetLength(params, 0);
  CheckEquals('0'#13#10, prog.ExecuteParam(params).Result.ToString);
  SetLength(params, 1);
  params[0] := 'hello';
  CheckEquals('1'#13#10'hellohello'#13#10, prog.ExecuteParam(params)
    .Result.ToString);
  SetLength(params, 2);
  params[1] := 123;
  CheckEquals('2'#13#10'hellohello'#13#10'123'#13#10, prog.ExecuteParam(params)
    .Result.ToString);
end;
...
begin
  prog := FCompiler.Compile('type TMyRec = record x, y : Integer; end;' +
    'function Hello : TMyRec;' + 'begin Result.x:=1; Result.y:=2; end;');
...
begin
  prog := FCompiler.Compile('type TScriptClass = class'#13#10 +
    'constructor Create;'#13#10 + 'destructor Destroy; override;'#13#10 +
    'end;'#13#10 +
...
procedure TCornerCasesTests.SubExprTest;
  ...
  procedure SubExprTree(output: TWriteOnlyBlockStream; const expr: TExprBase;
    indent: Integer);
  var
    i: Integer;
  begin
    ...
    if expr = nil then
      ...
      output.WriteString(expr.ClassName);
...
begin
  prog := FCompiler.Compile('function Test(a : Integer) : Integer;'#13#10 +
    'begin'#13#10 + 'Result:=a+1;'#13#10 + 'end;'#13#10 +
    'var s := "Hello";'#13#10 + 'Print(s);'#13#10 +
  ...
  CheckEquals('TBlockInitExpr'#13#10, MakeSubExprTree(testFuncSym.SubExpr[0]),
    'Test InitExpr');
  CheckEquals('TAssignExpr'#13#10 + #9'TIntVarExpr'#13#10 +
    #9'TAddIntExpr'#13#10 + #9#9'TIntVarExpr'#13#10 + #9#9'TConstIntExpr'#13#10,
    MakeSubExprTree(testFuncSym.SubExpr[1]), 'Test Expr');

  CheckEquals('TBlockInitExpr'#13#10 + #9'TInitDataExpr'#13#10 +
    #9#9'TStrVarExpr'#13#10, MakeSubExprTree((prog.GetSelf as TdwsProgram)
    .InitExpr), 'Main InitExpr');
  CheckEquals('TBlockExprNoTable3'#13#10 + #9'TAssignConstToStringVarExpr'#13#10
    + #9#9'TStrVarExpr'#13#10 + #9#9'nil'#13#10 + #9'TMagicProcedureExpr'#13#10
    + #9#9'TStrVarExpr'#13#10 + #9'TIfThenExpr'#13#10 +
    #9#9'TRelNotEqualStringExpr'#13#10 + #9#9#9'TStrVarExpr'#13#10 +
    #9#9#9'TConstStringExpr'#13#10 + #9#9'TMagicProcedureExpr'#13#10 +
    #9#9#9'TFuncSimpleExpr'#13#10 + #9#9#9#9'TConstIntExpr'#13#10,
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('type TTest = class ' + 'constructor Create;' +
    'procedure Test;' + 'end;' + 'var t : TTest;' +
    'constructor TTest.Create; begin t:=Self; end;' +
    'procedure TTest.Test; begin Print(ClassName); end;' + 'TTest.Create;');
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('Print("Hello");' + 'var x: Integer;');
...
begin
  ...
  CheckEquals
    ('Syntax Error: RecompileInContect does not support units [line: 1, column: 1]'#13#10,
    prog.Msgs.AsInfo);
end;
...
begin
  ...
  prog := FCompiler.Compile('unit dummy;'#13#10 + 'interface;'#13#10 +
    'uses Internal;'#13#10 + 'implementation;'#13#10);
  FCompiler.Config.CompilerOptions := cDefaultCompilerOptions;
  ...
  try
    ...
    CheckEquals('['#13#10 + #9'{'#13#10 + #9#9'"Token" : "ttUNIT",'#13#10 +
      #9#9'"Symbol" : {'#13#10 + #9#9#9'"Class" : "TUnitMainSymbol",'#13#10 +
      #9#9#9'"Name" : "dummy"'#13#10 + #9#9'},'#13#10 +
      #9#9'"SubContexts" : ['#13#10 + #9#9#9'{'#13#10 +
      #9#9#9#9'"Token" : "ttINTERFACE",'#13#10 + #9#9#9#9'"Symbol" : {'#13#10 +
      #9#9#9#9#9'"Class" : "TUnitMainSymbol",'#13#10 +
      #9#9#9#9#9'"Name" : "dummy"'#13#10 + #9#9#9#9'},'#13#10 +
      #9#9#9#9'"SubContexts" : ['#13#10 + #9#9#9#9#9'{'#13#10 +
      ...
      #9#9#9#9#9#9'"End" : " [line: 3, column: 14]"'#13#10 + #9#9#9#9#9'}'#13#10
      + #9#9#9#9'],'#13#10 + #9#9#9#9'"Start" : " [line: 2, column: 1]",'#13#10
      + #9#9#9#9'"End" : " [line: 4, column: 1]"'#13#10 + #9#9#9'},'#13#10 +
      #9#9#9'{'#13#10 + #9#9#9#9'"Token" : "ttIMPLEMENTATION",'#13#10 +
      #9#9#9#9'"Symbol" : {'#13#10 +
      #9#9#9#9#9'"Class" : "TUnitMainSymbol",'#13#10 +
      #9#9#9#9#9'"Name" : "dummy"'#13#10 + #9#9#9#9'},'#13#10 +
      #9#9#9#9'"SubContexts" : [ ],'#13#10 +
      #9#9#9#9'"Start" : " [line: 4, column: 1]",'#13#10 +
      #9#9#9#9'"End" : " [line: 6, column: 1]"'#13#10 + #9#9#9'}'#13#10 +
      #9#9'],'#13#10 + #9#9'"Start" : " [line: 1, column: 1]",'#13#10 +
      #9#9'"End" : " [line: 6, column: 1]"'#13#10 + #9'}'#13#10 + ']',
...
begin
  ...
  CheckEquals('Syntax Error: Missing resource [line: 1, column: 5]'#13#10,
    prog.Msgs.AsInfo);
...
begin
  ...
  prog := FCompiler.Compile(s);
  CheckEquals('Syntax Error: Unknown name "bug" [line: 1, column: 6011]'#13#10,
    prog.Msgs.AsInfo);
end;
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('Procedure Proc;'#13#10 + 'Begin'#13#10 +
    ' Try'#13#10 + '  Raise Exception.Create("");'#13#10 + ' Except'#13#10 +
    '  Proc;'#13#10 + ' End;'#13#10 + 'End;'#13#10 + 'Proc;');

  exec := prog.CreateNewExecution;
  try
    exec.Execute;
    CheckEquals
      ('Runtime Error: Maximal exception depth exceeded (11 nested exceptions) [line: 4, column: 29]'#13#10
      + ' [line: 9, column: 1]'#13#10, exec.Msgs.AsInfo, 'exception depth');
  finally
    exec := nil;
  end;
end;
...
begin
  ...
  try
    ...
    CheckEquals
      ('Warning: "Hello" has been deprecated: world [line: 1, column: 6]'#13#10,
      prog.Msgs.AsInfo);
...
begin
  ...
  try
    ...
    prog := FCompiler.Compile('{$F "test.dummy"');
    CheckEquals('Syntax Error: "}" expected [line: 1, column: 5]'#13#10,
      prog.Msgs.AsInfo);
  finally
    FCompiler.OnInclude := nil;
    filter.Free;
...
begin
  ...
  CheckEquals
    ('Syntax Error: There is no filter assigned to TDelphiWebScriptII.Config.Filter [line: 1, column: 5]'#13#10,
    prog.Msgs.AsInfo);
end;
...
var
  ...
  func: IInfo;
begin
  prog := FCompiler.Compile('unit test;'#13#10 + 'interface'#13#10 +
    'procedure Test;'#13#10 + 'implementation'#13#10 + 'procedure Test;'#13#10 +
    'var myvar : Integer;'#13#10 + 'begin'#13#10 + '   myVar := 1;'#13#10 +
    '   myVar:=2*myvar-StrToInt(IntToStr(myvar));'#13#10 +
    '   PrintLn(myVar);'#13#10 + 'end;'#13#10 + 'end.'#13#10);
  exec := prog.BeginNewExecution;
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('var o := new TObject;'#13#10 + 'ReExec;'#13#10 +
    'Print(''Here'');');
  CheckEquals('', prog.Msgs.AsInfo);

  exec := prog.Execute;
  CheckEquals('Runtime Error: Script is already running'#13#10,
    exec.Msgs.AsInfo);
...
begin
  prog := FCompiler.Compile
    ('try HostExcept; except Print("gobbled"); end;'#13#10 +
    'HostExcept;'#13#10);
  CheckEquals('', prog.Msgs.AsInfo);

  exec := prog.Execute;
  CheckEquals('Runtime Error: boom in HostExcept [line: 2, column: 1]'#13#10,
    exec.Msgs.AsInfo);
...
const
  cCase1 = 'type TTest = class'#10 + 'procedure Hello;'#10 +
    'procedure Hello(i : Integer); overload; begin end;'#10 + 'end;'#10 +
    'procedure TTest.Hello; begin end;';
  cCase2 = 'type TTest = class'#10 +
    'procedure Hello(i : Integer); overload; begin end;'#10 +
    'procedure Hello;'#10 + 'end;'#10 + 'procedure TTest.Hello; begin end;';
var
  prog: IdwsProgram;
  opts: TCompilerOptions;
begin
  ...
  try
    ...
    CheckEquals
      ('Hint: Overloaded method "Hello" should be marked with the "overload" directive [line: 2, column: 11]',
      Trim(prog.Msgs.AsInfo));
    prog := FCompiler.Compile(cCase2);
    CheckEquals
      ('Hint: Overloaded method "Hello" should be marked with the "overload" directive [line: 3, column: 11]',
      Trim(prog.Msgs.AsInfo));
    ...
    prog := FCompiler.Compile(cCase1);
    CheckEquals
      ('Syntax Error: Overloaded method "Hello" must be marked with the "overload" directive [line: 2, column: 11]',
      Trim(prog.Msgs.AsInfo));
    prog := FCompiler.Compile(cCase2);
    CheckEquals
      ('Syntax Error: Overloaded method "Hello" must be marked with the "overload" directive [line: 3, column: 11]',
      Trim(prog.Msgs.AsInfo));
  finally
    FCompiler.Config.CompilerOptions := opts;
  end;
...
begin
  prog := FCompiler.Compile('type TZ = Integer;'#13#10 +
    'const z = TZ(0);'#13#10 + 'var a : array [0..1] of Integer;');
...
begin
  prog := FCompiler.Compile('type TTest = class'#13#10 +
    'class var a external "alpha" : String;'#13#10 + 'end;');
...
begin
  prog := FCompiler.Compile('type TColor = Integer;'#13#10 +
    'type TTest = class'#13#10 + 'Field : TColor;'#13#10 +
    'property Prop : TColor read Field;'#13#10 +
    'property Prop2 : TColor;'#13#10 + 'end;');
...
begin
  prog := FCompiler.Compile('var toto : string = "test";'#13#10 +
    'type tobj = class(Tobject)'#13#10 + 'Destructor destroy;override;'#13#10 +
    'begin Print(toto) end;'#13#10 + 'end;'#13#10 + 'tobj.create.free;');
...
begin
  prog := FCompiler.Compile('var toto : string = "test";'#13#10 +
    'type tobj = class(Tobject)'#13#10 + 'Destructor destroy;override;'#13#10 +
    'begin Print(toto) end;'#13#10 + 'end;'#13#10 +
    'procedure Test; begin var o := tobj.create; end;'#13#10 + 'Test');
...
begin
  prog := FCompiler.Compile('type tobj = class(Tobject)'#13#10 +
    'Field : String;'#13#10 +
    'property Prop : String read Field default "hello";'#13#10 + 'end;');
...
begin
  prog := FCompiler.Compile
    ('unit Dummy; interface implementation initialization'#13#10 +
    'Assert(False);'#13#10 + 'finalization'#13#10 + 'PrintLn("here");'#13#10
    + 'end.');
  ...
  CheckEquals('Runtime Error: Assertion failed [line: 2, column: 1]'#13#10,
    exec.Msgs.AsInfo);
end;
...
begin
  prog := FCompiler.Compile
    ('unit Dummy; interface implementation initialization finalization'#13#10 +
    'Assert(False);');
...
begin
  ...
  prog := FCompiler.Compile('var a : array of Integer;'#13#10 + 'a.cleaR;');

  CheckEquals
    ('Hint: "cleaR" does not match case of declaration ("Clear") [line: 2, column: 3]'#13#10,
    prog.Msgs.AsInfo, 'array');
...
begin
  ...
  CheckEquals(TdwsCompilerExecution.ClassName,
    c.CompileTimeExecution.ClassName, 'Exec');
end;
...
begin
  ...
  prog := FCompiler.Compile('var a := 1;'#13#10 + '{$R "abort"}'#13#10 +
    'bug bug bug');

  CheckEquals('Syntax Error: Compilation aborted [line: 3, column: 1]'#13#10,
    prog.Msgs.AsInfo, 'array');
...
begin
  prog := FCompiler.Compile('unit Test; '#13#10 + 'interface'#13#10 +
    'implementation'#13#10 + 'initialization'#13#10 + 'Print("hello");'#13#10 +
    'finalization');
...
begin
  prog := FCompiler.Compile
    ('type TA = class procedure A; virtual; abstract; end;'#13#10 +
...
begin
  prog := FCompiler.Compile('var a : array of String;'#13#10 +
    'var j : JSONVariant;'#13#10 + 'for var n in a do j[n] := ...');
  // no expected leak
  prog := nil;
end;
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('PrintLn(#'''#13#10 + '   hello'#13#10 +
    '   world'');');
  exec := prog.CreateNewExecution;
  exec.Execute(0);
  CheckEquals('hello'#13#10'world'#13#10, exec.Result.ToString, 'CRLF');

  prog := FCompiler.Compile('PrintLn(#'''#10 + '   hello'#10 + '   world'');');
  exec := prog.CreateNewExecution;
  exec.Execute(0);
...
begin
  ...
  try
    ...
    CheckEquals
      ('[{"text":"hello","type":"Hint","pos":{"file":"*MainModule*","line":1,"col":3},"hintLevel":"Normal"},'
      + '{"text":"Unexpected \"..\"","type":"SyntaxError","pos":{"file":"*MainModule*","line":2,"col":2}}]',
      wr.ToString);
  finally
    wr.Free;
  end;
...
begin
  prog := FCompiler.Compile
    ('{$HINT "hello"}'#13#10'{$WARNING "world"}'#13#10'...');
  CheckEquals(3, prog.Msgs.Count, prog.Msgs.AsInfo);
  n := 0;
  for msg in prog.Msgs do
  begin
    case n of
      0:
        CheckIs(msg, THintMessage);
      1:
        CheckIs(msg, TWarningMessage);
      2:
        CheckIs(msg, TSyntaxErrorMessage);
    end;
    Inc(n);
  end;
end;
...
begin
  ...
  CheckEquals
    ('Warning: Unit name does not match file name [line: 1, column: 6]'#13#10,
    prog.Msgs.AsInfo);
  ...
  prog := FCompiler.Compile('unit Hello.World;', 'Hello.world.pas');
  CheckEquals
    ('Hint: Unit name case does not match file name [line: 1, column: 6]'#13#10,
    prog.Msgs.AsInfo);
...
begin
  prog := FCompiler.Compile
    ...
    + 'PrintLn(Test);');
  CheckEquals
    ('Runtime Error: User defined exception: hello [line: 1, column: 62]'#13#10
    + ' [line: 2, column: 9]'#13#10, prog.Execute.Msgs.AsInfo);

  prog := FCompiler.Compile
    ('procedure Stuff; begin raise Exception.Create("world"); end;'#13#10 +
    'function Test : String; begin Stuff; end;'#13#10 + 'PrintLn(Test);');
  CheckEquals
    ('Runtime Error: User defined exception: world [line: 1, column: 55]'#13#10
    + 'Test [line: 2, column: 31]'#13#10 + ' [line: 3, column: 9]'#13#10,
...
begin
  prog := FCompiler.Compile
    ('procedure Test(a : Boolean); begin if a then Print(a) else ; end;');
  E := prog.Table.FindSymbol('Test', cvMagic).AsFuncSymbol.SubExpr[1];
  CheckEquals('TIfThenExpr', E.ClassName, 'empty else');

  prog := FCompiler.Compile
    ('procedure Test(a : Boolean); begin if a then Print(a) else Print(a); end;');
  E := prog.Table.FindSymbol('Test', cvMagic).AsFuncSymbol.SubExpr[1];
  CheckEquals('TIfThenElseExpr', E.ClassName);

  prog := FCompiler.Compile
    ('procedure Test(a : Boolean); begin if a then else Print(a); end;');
  E := prog.Table.FindSymbol('Test', cvMagic).AsFuncSymbol.SubExpr[1];
...
var
  ...
  code: String;
begin
  code := 'Print(1_);'#10 + 'Print(1_2);'#10 + 'Print(1_2_3);'#10 +
    'Print(4_.5);'#10 + 'Print(4_5.6);'#10 + 'Print(4_5_6.7);';
...
begin
  ...
  try
    prog := FCompiler.Compile
      ...
      + 'begin callback(a, b, c); end;'#13#10 + 'var va : TObject;'#13#10 +
      'var vb : String;'#13#10 + 'var vc : Variant;'#13#10 +
...
var
  ...
  exec: IdwsProgramExecution;
begin
  prog := FCompiler.Compile('function Test(k : Integer) : String; begin'#13#10 +
    'if k <= 0 then exit "done";'#13#10 + 'Print(k);'#13#10 +
    'Result := "," + RoundTrip("Test", k);'#13#10 + 'end;'#13#10 +
    'PrintLn(Test(5));');
  CheckEquals('', prog.Msgs.AsInfo);
  try
    exec := prog.CreateNewExecution;
...
var
  prog: IdwsProgram;
begin
  prog := FCompiler.Compile('unit Test; interface implementation'#10 +
    'initialization'#10 + 'end');
  CheckEquals('Warning: Dot "." expected [line: 3, column: 1]'#13#10,
    prog.Msgs.AsInfo);
  prog := FCompiler.Compile('unit Test; interface implementation'#10 +
    'initialization'#10 + 'finalization'#10 + 'end');
  CheckEquals('Warning: Dot "." expected [line: 4, column: 1]'#13#10,
    prog.Msgs.AsInfo);
end;
