...
uses Windows, Classes, SysUtils, dwsXPlatformTests, dwsComp, dwsScriptSource,
  dwsTokenizer, dwsTokenTypes, dwsXPlatform, dwsErrors, dwsUtils,
  dwsPascalTokenizer, TypInfo;
...
// DoActionStream
//
procedure TTokenizerTests.DoActionStream(Sender: TTokenizer;
...
begin
  ...
  if action = caName then
    tt := ' (' + GetEnumName(TypeInfo(TTokenType),
      Ord(Sender.RawTokenBufferNameToType)) + ')';

  FActions.Add(GetEnumName(TypeInfo(TConvertAction), Ord(action)) + ' at ' +
    Format('%d,%d', [Sender.CurrentPos.Line, Sender.CurrentPos.Col]) + ' from '
    + Format('%d,%d', [tok.FScriptPos.Line, tok.FScriptPos.Col]) + ' on <<' +
    Sender.RawTokenBuffer + '>>' + tt);
end;

// TokenizeToActions
//
procedure TTokenizerTests.TokenizeToActions(tok: TTokenizer;
  const code: String);
begin
  FActions.Clear;
...
begin
  FSourceFile.code :=
    'var i:=1; // hello'#10'i += 1; (* world *) begin'#10#10'PrintLn(i);end;';
  rules := TPascalTokenizerStateRules.Create;
  t := rules.CreateTokenizer(FMsgs, nil);
  try
    TokenizeToActions(t,
    ...
    CheckEquals('caName at 1,4 from 1,1 on <<var>> (ttVAR)'#13#10 +
      ...
      'caName at 4,16 from 4,15 on <<;>> (ttSEMI)'#13#10, FActions.Text);
  finally
    t.Free;
    rules.Free;
  end;
