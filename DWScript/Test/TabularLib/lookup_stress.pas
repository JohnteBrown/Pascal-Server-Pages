...
var
tabNoJIT := TabularData.CreateFromDataSet(db.Query('select * from test'),
  ['nojit']);

var
tabJIT := TabularData.CreateFromDataSet(db.Query('select * from test'),
  ['jit']);

var
fields = ['"a" 10 {"a1":1,"a2":2}',
  '"b" 100 {"b1":107,"b2":113,"b3":119,"b4":123}', '"c" 1000 {"c1":10000}'];

var
operators = ['+', '*', 'max', 'min', 'relu'];
...
begin
  ...
  var
  ref := tabNoJIT.EvaluateAggregate('sum', ops).ToString;
  var
  eval := tabJIT.EvaluateAggregate('sum', ops).ToString;
  if eval <> ref then
  begin
