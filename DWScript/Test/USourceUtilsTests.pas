...
type
  ...
  private
    ...
    function NeedUnitHandler(const unitName: String; var unitSource: String)
      : IdwsUnit;
...
begin
  prog := FCompiler.Compile('var printit : Boolean;'#13#10 + 'PrintL');
...
begin
  prog := FCompiler.Compile
    ('type TMyClass = class constructor Create; procedure Test; end;'#13#10 +
    'procedure TMyClass.Test;begin'#13#10 + 'Self.Create');
  ...
  CheckEquals('TMyClass', (sugg.Symbols[0] as TMethodSymbol).StructSymbol.Name,
    'Create 9,0 struct');

  prog := FCompiler.Compile
    ('type TMyClass = Class(TObject) Field : TMyClass; procedure first; procedure second; procedure third; End; '
    + 'procedure TMyClass.first; begin '#13#10 + 'Self.Field.');
...
begin
  ...
  sugg := TdwsSuggestions.Create(prog, scriptPos, [soNoReservedWords]);
  var
    expected: TArray<String> := ['TClass', 'TComplex', 'TCustomAttribute',
      'TJPEGOption', 'TJPEGOptions', 'TObject', 'TPixmap', 'TRTTIRawAttribute',
      'TRTTIRawAttributes', 'TRTTITypeInfo', 'TSourceCodeLocation', 'TVector'];
...
begin
  CheckTrue((unitName = 'SomeUnit') or (unitName = 'Test'),
    'Only the units ''SomeUnit'' & ''Test'' are handled properly!');
  if unitName = 'SomeUnit' then
    unitSource := 'unit SomeUnit;'
  else
    unitSource := FTestUnit;
end;
...
begin
  ...
  prog := FCompiler.Compile
    ('procedure Dummy;'#13#10'begin begin'#13#10#13#10'end end'#13#10);
...
begin
  ...
  var
  expected := ['Contains', 'Count', 'High', 'IndexOf', 'Length', 'Low'];
...
begin
  prog := FCompiler.Compile
    ('const s : array [1..2] of record xy : Integer end = [('#13#10 + ' )];');
...
begin
  ...
  var
  expected := ['Add', 'Clear', 'Contains', 'Copy', 'Count', 'Delete', 'Filter',
    'High', 'IndexOf', 'Insert', 'Length', 'Low', 'Map', 'Move', 'Peek', 'Pop',
    'Push', 'Remove', 'Reverse', 'SetLength', 'Sort', 'Swap'];
...
begin
  prog := FCompiler.Compile
    ('type TObj = class X : Integer; end; var a : array of TObj;'#13#10
    + 'a[0].');
...
const
  cSugg: array [0 .. 20] of String = ('Abs', 'Clamp', 'Compare', 'Compare',
    'Factorial', 'Hello', 'IsOdd', 'IsPrime', 'LeastFactor', 'Max', 'Min',
    'Next', 'PopCount', 'Sign', 'Sqr', 'TestBit', 'ToBin', 'ToHexString',
    'ToString', 'ToString', 'Unsigned32');
...
begin
  prog := FCompiler.Compile
    ('type TIntegerHelper = helper for Integer const Hello = 123; ' +
    'function Next : Integer; begin Result:=Self+1; end; end;'#13#10 +
    'var d : Integer;'#13#10 + 'd.Nex');
...
procedure TSourceUtilsTests.FunctionReturningArrayTest;
  ...
  begin
    ...
    scriptPos := TScriptPos.Create(prog.SourceList[0].SourceFile, 2,
      Length(Code) - Pos(#10, Code) + 1);
    sugg := TdwsSuggestions.Create(prog, scriptPos, [soNoReservedWords]);
...
begin
  ...
  CheckIsJoin
    ('function Test : array of String; begin end;'#13#10'Test.Map(Trim).Jo');
end;
...
const
  cSugg: array [0 .. 12] of String = ('Add', 'Clone', 'Defined', 'Delete',
    'ElementName', 'Extend', 'High', 'Length', 'Low', 'Push', 'Swap',
    'ToString', 'TypeName');
...
begin
  ...
  try
    ...
    prog := FCompiler.Compile('var v : JSONVariant;'#13#10 + 'v.');
...
begin
  ...
  CheckEquals('SomeUnit', sugg.Code[0],
    'The suggestion should be the unit ''SomeUnit''');
  ...
  sugg := TdwsSuggestions.Create(prog, scriptPos,
    [soNoReservedWords, soNoUnits]);

  CheckEquals(0, sugg.Count,
    'There shouldn''t be units in the suggestions at all');
...
begin
  prog := FCompiler.Compile('var a : array of String;'#10 +
    'for var test1 := 0 to 10 do'#10 + 'for var test2 in a do begin'#10 +
    'teS'#10 + 'end;');
...
begin
  prog := FCompiler.Compile('var i : Integer;'#13#10 +
    'if i>0 then Inc(i);'#13#10 + 'function Test : Integer;'#13#10 +
  ...
  CheckTrue(prog.ProgramObject.Expr.ReferencesVariable(sym),
  ...
  CheckFalse((funcExec.GetSelf as TdwsProgram).Expr.ReferencesVariable(sym),
    'not referenced in test');
end;
...
var
  ...
  scriptPos: TScriptPos;
begin
  prog := FCompiler.Compile('try'#13#10 + 'except'#13#10 +
    'on e : Exception do'#13#10 + 'e.s'#13#10);
...
begin
  ...
  enum := (prog.Table.FindTypeSymbol('TContinuous', cvPublic)
    as TEnumerationSymbol);
  ...
  enum := (prog.Table.FindTypeSymbol('TContinuous2', cvPublic)
    as TEnumerationSymbol);
  ...
  enum := (prog.Table.FindTypeSymbol('TDiscontinuous', cvPublic)
    as TEnumerationSymbol);
...
begin
  ...
  for i := 1 to 100 do
  begin
    CheckEquals(i - 1, (enum.Elements.FindLocal('v' + FastInt64ToStr(i))
      as TElementSymbol).Value, 'value of ' + IntToStr(i - 1));
    CheckEquals('v' + IntToStr(i), enum.ElementByValue(i - 1).Name,
      'name of ' + IntToStr(i - 1));
  end;
end;
...
begin
  prog := FCompiler.Compile('type TTest = (One);'#13#10 +
    'Print(TTest.One.Name);'#13#10 + 'var i : TTest;'#13#10 +
    'Print(i.Value);'#13#10 + 'Print(i.QualifiedName);'#13#10);
...
begin
  prog := FCompiler.Compile('type TTest = (One);'#13#10 +
    'type TSet = set of TTest;'#13#10 + 'var i : TSet;'#13#10 + 'i.');
...
begin
  prog := FCompiler.Compile('type TTest = class static public'#13#10 +
    ...
    + 'property Test[i : Integer] : String read GetTest;'#13#10 + 'end;'#13#10 +
    'Print(TTest.GetTest(1));'#13#10 + 'Print(TTest.Test[2]);'#13#10);
...
begin
  prog := FCompiler.Compile
    ('type TTest = class private fb : Boolean; protected fi : Integer; fs : String; public'#13#10
    + 'class const fc = 123;'#13#10 + 'property F : Integer read '#13#10 + 'f');
...
begin
  prog := FCompiler.Compile
    ('type TTest = record xyz, abc : Integer; function Foo : String; end;'#13#10
    + 'const c : array of TTest = [('#13#10 + 'x');
...
var
  ...
  scriptPos: TScriptPos;
begin
  prog := FCompiler.Compile('with v := 1 do'#13#10 + 'Print(v.ToS'#13#10);
...
begin
  prog := FCompiler.Compile
    ('function Toto(s: string): string; overload; begin Result := s; end;'#13#10
...
var
  ...
  scriptPos: TScriptPos;
begin
  prog := FCompiler.Compile('var s : String;'#13#10 + 'PrintLn(s.Length.tos');
...
begin
  prog := FCompiler.Compile('type TTest = class'#10 +
    ...
    'end;'#10 + 'var t : TTest;'#10 + 'PrintLn(t[0].ToU');
  ...
  prog := FCompiler.Compile('type TTest = class'#10 +
    ...
    '   property Prop[i : Integer] : String read Get; default;'#10 + 'end;'#10 +
    'PrintLn(TTest[0].ToL');
...
begin
  prog := FCompiler.Compile('type TTest = class ' +
    ...
    'property Hello : String read Func description "world"; ' + 'end;');
  ...
  prop := (cls as TClassSymbol).Members.FindLocalOfClass('Hello',
    TPropertySymbol);
...
begin
  FTestUnit := 'unit test; interface implementation'#13#10 +
    'var zxy : String;'#13#10 + 'procedure zzz; begin '#13#10 + 'zzz; end;';
  ...
  prog := FCompiler.Compile('uses Test;'#13#10 + 'var za := 0;'#13#10 +
    'za := 1;');
...
var
  ...
  scriptPos: TScriptPos;
begin
  prog := FCompiler.Compile('var myvariable: string;'#10 + #10 +
    'procedure foo(const att: Boolean);'#10 + 'var'#10 + '  lcl: Boolean;'#10 +
    'begin'#10 + '  {here}'#10 + 'end;'#10);
...
begin
  ...
  CheckEquals
    ('function Test1(a: Integer; b: Integer; const c: String; var d: Boolean; e: type Boolean): String',
    prog.Table.FindSymbol('Test1', cvMagic).Description, 'Test1');
  CheckEquals
    ('function Test2(const a: String; const b: String; c: String; var d: Boolean; var e: Boolean): String',
    prog.Table.FindSymbol('Test2', cvMagic).Description, 'Test2');
  CheckEquals
    ('procedure Test3(a: Integer; b: Integer; c: type Integer; d: Boolean; var e: Boolean)',
    prog.Table.FindSymbol('Test3', cvMagic).Description, 'Test3');
  CheckEquals
    ('procedure Test4(var a: Integer; var b: Integer; const c: Integer; d: Boolean; e: Boolean)',
    prog.Table.FindSymbol('Test4', cvMagic).Description, 'Test4');

  symClassFn := (prog.Table.FindLocal('TTest') as TClassSymbol)
    .Members.FindSymbol('TestFC', cvMagic);
...
begin
  prog := FCompiler.Compile
    ...
    + 'type TStringArray = array of String;'#10 + 'type TNameValue = record'#10
    + '   Name : String;'#10 + '   Value : String;'#10 + 'end;'#10 +
    'type TMyClass = class'#10 + '   private'#10 +
    '      FField : array[String] of TTestEnum;'#10 + '   public'#10 +
    '      property Prop : array of TTestEnum;'#10 +
    '      property Keys : array of String read (FField.Keys);'#10 +
    '      property NameValue : TNameValue;'#10 +
    '      property StringTab : TStringArray;'#10 + 'end;');
  CheckFalse(prog.Msgs.HasErrors, prog.Msgs.AsInfo);
  classSymMembers := (prog.Table.FindLocal('TMyClass') as TClassSymbol).Members;

  CheckEquals('property Prop: array of TTestEnum read write',
    classSymMembers.FindSymbol('Prop', cvMagic).Description, 'Test1');
  CheckEquals('property Keys: array of String read',
    classSymMembers.FindSymbol('Keys', cvMagic).Description, 'Test2');
  CheckEquals('property NameValue: TNameValue read write',
    classSymMembers.FindSymbol('NameValue', cvMagic).Description, 'Test3');
  CheckEquals('property StringTab: TStringArray read write',
    classSymMembers.FindSymbol('StringTab', cvMagic).Description, 'Test4');
end;
...
var
  prog: IdwsProgram;
begin
  prog := FCompiler.Compile('const i : Integer = 123;'#10 + 'const f = 1.5;'#10
    + 'const s = ''foobar'';'#10 + 'const s2 = "foo''bar";'#10 +
    'const v : Variant = Null;'#10);
  CheckFalse(prog.Msgs.HasErrors, prog.Msgs.AsInfo);

  CheckEquals('const i: Integer = 123',
    (prog.Table.FindLocal('i') as TConstSymbol).Description);
  CheckEquals('const f: Float = 1.5',
    (prog.Table.FindLocal('f') as TConstSymbol).Description);
  CheckEquals('const s: String = ''foobar''',
    (prog.Table.FindLocal('s') as TConstSymbol).Description);
  CheckEquals('const s2: String = "foo''bar"',
    (prog.Table.FindLocal('s2') as TConstSymbol).Description);
  CheckEquals('const v: Variant = Null',
    (prog.Table.FindLocal('v') as TConstSymbol).Description);
end;
...
begin
  prog := FCompiler.Compile
    ...
    'type TTestSet = set of TTestEnum;'#10 + 'const s1 : TTestSet = [];'#10 +
    'const s2 : TTestSet = [ TTestEnum.Alpha ];'#10 +
    'const s3 : TTestSet = [ TTestEnum.Beta, TTestEnum.Gamma ];'#10 +
    'const s4 : TTestSet = [ TTestEnum(10) ];'#10);
  CheckFalse(prog.Msgs.HasErrors, prog.Msgs.AsInfo);

  CheckEquals('const s1: TTestSet = []',
    (prog.Table.FindLocal('s1') as TConstSymbol).Description);
  CheckEquals('const s2: TTestSet = [ TTestEnum.Alpha ]',
    (prog.Table.FindLocal('s2') as TConstSymbol).Description);
  CheckEquals('const s3: TTestSet = [ TTestEnum.Beta, TTestEnum.Gamma ]',
    (prog.Table.FindLocal('s3') as TConstSymbol).Description);
  CheckEquals('const s4: TTestSet = [ TTestEnum(10) ]',
    (prog.Table.FindLocal('s4') as TConstSymbol).Description);
end;
...
var
  ...
  scriptPos: TScriptPos;
begin
  prog := FCompiler.Compile('begin'#13#10 + 'var xyz := "";'#13#10 + 'x');

  CheckNotEquals(String(''), prog.Msgs.AsInfo,
    'should have compiled with errors');
...
begin
  ...
  try
    lines.Text := 'unit Unit1;'#13#10 + 'interface'#13#10 +
      'procedure Test(const A, Blah: String; const C: string); overload;'#13#10
      + 'procedure Test; overload;'#13#10 + 'implementation'#13#10 +
      'procedure test(const A, Blah: string; const C: String);'#13#10 +
      'begin end;'#13#10 + 'procedure test;'#13#10 + 'begin end;';
    ...
    try
      NormalizeSymbolsCase(lines, prog.SourceList[0].SourceFile,
        prog.SymbolDictionary, normalizer.Normalize);
      CheckEquals
        ('3, 48, String'#13#10'6, 11, Test'#13#10'6, 31, String'#13#10'8, 11, Test'#13#10,
        normalizer.Text);
    finally
      normalizer.Free;
    end;
...
begin
  lines := TStringList.Create;
  try
    lines.Text := 'function Test(a : Integer) : Integer;'#13#10 + 'begin'#13#10
      + 'Exit(a);'#13#10 + 'end;';
...
begin
  ...
  try
    ...
    try
      NormalizeSymbolsCase(lines, prog.SourceList[0].SourceFile,
        prog.SymbolDictionary, normalizer.Normalize);
      CheckEquals('type TProc = procedure;'#13#10 +
        'type TFunc = function(proc : TProc): TProc;'#13#10 +
        'var f : TFunc;'#13#10, lines.Text);
    finally
      normalizer.Free;
    end;
  finally
...
begin
  ...
  try
    ...
    try
      NormalizeSymbolsCase(lines, prog.SourceList[0].SourceFile,
        prog.SymbolDictionary, normalizer.Normalize);
      CheckEquals('var s : String; var a : array of String;'#13#10 +
        'PrintLn(s.Length + s.High + s.Low + a.Length + a.High + a.Low);'#13#10,
        lines.Text);
    finally
      normalizer.Free;
  ...
  try
    ...
    try
      ...
      CheckEquals
        ...
        'for var e in TEnum do PrintLn(Integer(e));'#13#10 +
        'for var j in a do PrintLn(a[j]);'#13#10 +
        'for i in a do PrintLn(a[i].ToString);'#13#10, lines.Text);
    finally
      normalizer.Free;
    end;
  finally
  ...
  try
    ...
    try
      ...
      CheckEquals
        ('var b := True; begin if not B then b := False else b := nil <> nil; end;'#13#10
        + 'while b do break;'#13#10 +
        'for var i := 0 to 1 do case I of 1 : ; else end;'#13#10, lines.Text);
    finally
      normalizer.Free;
    end;
  finally
...
begin
  ...
  try
    lines.Text :=
      ...
      't.hello := "a";'#13#10 + 't.&hello := "b";'#13#10 +
      't.world := True;'#13#10 + 't.&world := False;'#13#10;
    ...
    try
      ...
      CheckEquals
        ('var t := record "Hello" : String; &World : Boolean; end;'#13#10 +
        't.Hello := "a";'#13#10 + 't.&Hello := "b";'#13#10 +
        't.World := True;'#13#10 + 't.&World := False;'#13#10, lines.Text);
    finally
      normalizer.Free;
    end;
  finally
...
begin
  lines := TStringList.Create;
  try
    lines.Text := 'var i : Integer;'#13#10 + 'iNc(i);'#13#10 + 'dec(i);'#13#10;
    ...
    try
      NormalizeSymbolsCase(lines, prog.SourceList[0].SourceFile,
        prog.SymbolDictionary, normalizer.Normalize);
      CheckEquals('var i : Integer;'#13#10 + 'Inc(i);'#13#10 + 'Dec(i);'#13#10,
        lines.Text);
    finally
      normalizer.Free;
    end;
...
begin
  ...
  CheckTrue((sym as TCompositeTypeSymbol).MembersVisibilities = [cvPrivate,
    cvPublic], 'private class');
  ...
  CheckTrue((sym as TCompositeTypeSymbol).MembersVisibilities = [cvPublic,
    cvPublished], 'published class');

  sym := prog.Table.FindTypeLocal('TPublicProtected');
  CheckTrue((sym as TCompositeTypeSymbol).MembersVisibilities = [cvPublic,
    cvProtected], 'public protected class');
end;
