...
const
  cData = #13#10 +
    ...
    'Content-Disposition: form-data; name="text"'#13#10 + #13#10 +
    'text default' +
    #13#10'-----------------------------9051914041544843365972754266'#13#10 +
    'Content-Disposition: form-data; name="file1"; filename="a.txt"'#13#10 +
    'Content-Type: text/plain'#13#10 + #13#10 + 'Content of a.txt.'#13#10 +
    #13#10'-----------------------------9051914041544843365972754266'#13#10 +
...
begin
  WebUtils.ParseMultiPartFormData(cData,
    '-----------------------------9051914041544843365972754266', data);
  ...
  CheckEquals('Content-Disposition: form-data; name="text"',
    data[0].RawHeaders);

  CheckEquals('Content of a.txt.'#13#10, data[1].RawData);
  CheckEquals('form-data; name="file1"; filename="a.txt"',
    data[1].ContentDisposition);
  CheckEquals('text/plain', data[1].ContentType);
  CheckEquals('file1', data[1].Name);
  CheckEquals('a.txt', data[1].FileName);

  CheckEquals('<!DOCTYPE html><title>Content of a.html.</title>',
    data[2].RawData);
  CheckEquals('form-data; name="file2"; filename="a.html"',
    data[2].ContentDisposition);
...
begin
  ...
  CheckEquals(Round((dt - 1 / 24) * 86400),
    Round(WebUtils.RFC822ToDateTime('Fri Feb 01 2022 07:08:09 GMT+0100')
    * 86400));
end;
...
begin
  ...
  CheckEquals('&amp;;/&lt;&gt;&quot;%', WebUtils.HTMLTextEncode('&;/<>"%'),
    'encode &;/<>"%');
  CheckEquals('&;/<>"%', WebUtils.HTMLTextDecode('&amp;;/&lt;&gt;&quot;%'),
    'decode 1 &;/<>"%');
  CheckEquals('&;/<>"%', WebUtils.HTMLTextDecode('&#38;;/&#x3C;&GT;&#x00022;%'),
    'decode 2 &;/<>"%');

  CheckEquals('&#39;&nbsp;', WebUtils.HTMLTextEncode(''''#$00A0),
    'encode NBSP apos');
  CheckEquals(#$00A0'''', WebUtils.HTMLTextDecode('&nbsp;&apos;'),
    'decode NBSP apos');

  CheckEquals('hello world',
    WebUtils.HTMLTextDecode('<span a="b">hello<b c=''jj''> </b>world</span>'),
    'decode with tag');

  CheckEquals('&apos &#zz ', WebUtils.HTMLTextDecode('&apos &#zz &#x0zz'),
    'broken entity');
...
begin
  ...
  CheckEquals('hello\"world', WebUtils.CSSTextEncode('hello"world'),
    'hello world');
end;
...
begin
  ...
  CheckEquals('&amp;;/&lt;&gt;&quot;%', WebUtils.XMLTextEncode('&;/<>"%'),
    'encode &;/<>"%');
  CheckEquals('&;/<>"%', WebUtils.XMLTextDecode('&amp;;/&lt;&gt;&quot;%'),
    'decode &;/<>"%');

  CheckEquals('&apos;'#$00A0, WebUtils.XMLTextEncode(''''#$00A0),
    'encode NBSP apos');
  CheckEquals(#$00A0'''', WebUtils.XMLTextDecode(#$00A0'&apos;'),
    'decode NBSP apos');
end;
...
begin
  ...
  CheckEquals('', WebUtils.DecodeURLEncoded('', 1, 0),
    'decode empty with length');
  CheckEquals('', WebUtils.DecodeURLEncoded('', 1), 'decode empty not length');

  CheckEquals('a b"c', WebUtils.DecodeURLEncoded('a%20b%22c', 1, 9),
    'decode a b"c full');
  CheckEquals('a b"c', WebUtils.DecodeURLEncoded('a%20b%22czzz', 1, 9),
    'decode a b"c partial');
end;
