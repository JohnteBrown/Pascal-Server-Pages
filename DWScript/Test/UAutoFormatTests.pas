...
begin
  ...
  CollectFiles(basePath + 'PropertyExpressionsPass' + PathDelim,
    cFilter, FTests);
  ...
  FAutoFormat := TdwsAutoFormat.Create
    (TdwsParser.Create(FTokRules.CreateTokenizer(nil, nil),
    FPascalRules.CreateRules));
end;
...
// DoInclude
//
procedure TAutoFormatTests.DoInclude(const scriptName: String;
...
// CheckAutoFormatted
//
procedure TAutoFormatTests.CheckAutoFormatted(const expected, actual,
  testLabel: String);
begin
  var
  ...
  try
    ...
    for var i := 0 to Max(slExpected.Count, slActual.Count) - 1 do
    begin
      CheckFalse(i >= slExpected.Count, 'More lines (' +
        IntToStr(slActual.Count) + ') than expected (' +
        IntToStr(slExpected.Count) + ') for ' + testLabel + #10 +
        slActual.Text);
      CheckFalse(i >= slActual.Count, 'Less lines (' + IntToStr(slActual.Count)
        + ') than expected (' + IntToStr(slExpected.Count) + ') for ' +
        testLabel + #10 + slActual.Text);
      CheckEquals(TrimRight(slExpected[i]), slActual[i],
        'Mistmatch at line ' + IntToStr(i + 1) + ' for ' + testLabel + #10 +
        slActual.Text);
    end;
  finally
    slExpected.Free;
...
begin
  ...
  try
    ...
    begin
      ...
      var
      processed := FAutoFormat.Process(source.Text);
      prog := FCompiler.Compile(processed,
        'Test\' + ExtractFileName(FTests[i]));

      CheckEquals(False, prog.Msgs.HasErrors, FTests[i] + #13#10 +
        prog.Msgs.AsInfo + #13#10 + processed);
    end;
...
begin
  ...
  try
    ...
    begin
      ...
      CheckEquals(False, prog.Msgs.HasErrors,
        FTests[i] + ' fail pre-processed compilation'#13#10 + prog.Msgs.AsInfo);

      exec := prog.Execute;
      var
      originalOutput := exec.Result.ToString;
      if exec.Msgs.Count > 0 then
        originalOutput := originalOutput + #13#10 + '>>> Runtime Error: ' +
          AsInfoWithoutPosition(exec.Msgs);
      ...
      CheckEquals(False, prog.Msgs.HasErrors,
        FTests[i] + ' fails post-formatting' + #13#10 + prog.Msgs.AsInfo +
        #13#10 + processed);
      ...
      if exec.Msgs.Count > 0 then
        output := output + #13#10 + '>>> Runtime Error: ' +
          AsInfoWithoutPosition(exec.Msgs);
...
begin
  for var i := 0 to FAutoFormatTests.Count - 1 do
  begin
    var
    code := LoadTextFromFile(FAutoFormatTests[i]);
    var
    expected := LoadTextFromFile(ChangeFileExt(FAutoFormatTests[i],
      '.fmt.pas'));
    CheckAutoFormatted(expected, FAutoFormat.Process(code),
      FAutoFormatTests[i]);
  end;
end;
...
begin
  ...
  begin
    ...
    expected := LoadTextFromFile(ChangeFileExt(FAutoFormatTests[i],
      '.fmt.pas'));
    var
    processed := FAutoFormat.Process(code);
    CheckAutoFormatted(expected, FAutoFormat.Process(processed),
      FAutoFormatTests[i]);
  end;
end;
...
begin
  CheckEquals('var i := 1;'#10'i := i + 1;'#10,
    FAutoFormat.Process(#9#9'var'#9'i:=1;i:=i   +   1   ;    '#10));
  CheckEquals('i := a + b;'#10,
    FAutoFormat.Process('i'#9' := '#9' a+'#9#9'b'#9'  ;'#9#9));
  CheckEquals('abc := +2;'#10, FAutoFormat.Process(' abc:=+2;'));
  CheckEquals('ab := 0.1e+10;'#10'cd := -45e-1;'#10,
...
begin
  CheckEquals('begin'#10#9'i := 2;'#10'end;'#10,
  ...
  CheckEquals('beginning := 1;'#10'begin'#10#9'ending := 2;'#10'end;'#10,
...
begin
  CheckEquals('repeat'#10#9'i += 1;'#10'until i >= 10;'#10,
...
begin
  CheckEquals('while i > 0 do'#10#9'i -= 1;'#10'Done()'#10,
    FAutoFormat.Process('While i>0 do i-=1;Done()'));
  CheckEquals('while i > 0 do begin'#10#9'i -= 1;'#10'end;'#10'Done()'#10,
...
begin
  CheckEquals('if b then'#10#9'doit()'#10'else dont();'#10'done()'#10,
  ...
  CheckEquals('for a in b do'#10#9'if c then'#10#9#9'd;'#10,
...
begin
  CheckEquals('procedure Hello;'#10'begin'#10#9'i := 1'#10'end;'#10#10,
  ...
  CheckEquals
    ...
    FAutoFormat.Process
    ('procedure PrintBool(v:Variant);begin PrintLn(if v then''True'' else''False'');end;')
    );
end;
...
begin
  CheckEquals('procedure Hello;'#10'begin'#10#9'i := 1'#10'end;'#10#10,
    FAutoFormat.Process('procedure Hello;begin i:=1 end;'));
  CheckEquals('if a then'#10#9'b(0)'#10'else c(1)'#10,
...
begin
  CheckEquals('s := '''';'#10'b := "Hello";'#10,
    FAutoFormat.Process('s:='''';b:="Hello";'));
  CheckEquals('s := #9"abc"#10'#10, FAutoFormat.Process('s:=#9"abc"#10'));
end;
...
begin
  CheckEquals('i := 1; // begin'#10'i := 2;'#10'// end'#10,
    FAutoFormat.Process('i:=1;// begin'#10'i:=2;'#10'// end'));
  CheckEquals('(* /* *)'#10'i := 1; // begin i:=1'#10'i *= 2 /*end*/ 1;'#10,
    FAutoFormat.Process
    ('(* /* *)'#10'i:=1; // begin i:=1'#10'i*=2/*end*/1;'#10));
  CheckEquals('a'#10#10'/* bla'#10'bla */'#10,
...
begin
  CheckEquals('{$ifdef A}'#10'b;'#10'{$endif}'#10,
    FAutoFormat.Process('{$ifdef A}'#10'b;'#10'{$endif}'#10));
  CheckEquals('{$ifdef A} b; {$endif}'#10,
...
begin
  CheckEquals('case a of'#10#9'-1..+1 : b'#10'end'#10,
...
begin
  CheckEquals('for i := 1 to 9 do'#10#9'PrintLn(i);'#10,
...
begin
  CheckEquals('// hello'#10'uses FooBar;'#10,
    FAutoFormat.Process('// hello'#10'uses FooBar;'));
  CheckEquals('// hello'#10#10'uses'#10#9'FooBar;'#10,
    FAutoFormat.Process('// hello'#10#10'uses'#10'FooBar;'));
end;
...
begin
  CheckEquals('if 1 then begin'#10#9'foo'#10'end'#10,
    FAutoFormat.Process('if 1 then'#10'begin'#10'foo'#10'end'));
end;
...
begin
  CheckEquals('a :='#10#9'b;'#10, FAutoFormat.Process('a :='#10#9'b;'#10));
  CheckEquals('var a :='#10#9'b;'#10,
    FAutoFormat.Process('var a :='#10#9'b;'#10));
end;
...
begin
  CheckEquals('MyFunc12345678901234567890123('#10 +
    #9'longFunc12345678901234567890123456789('#10 +
    #9#9'param12345678901234567890123456789012345678901234567890'#10 + #9')'#10
    + ')'#10, FAutoFormat.Process
...
begin
  CheckEquals('a :='#10 + #9'    hello'#10 + #9'and world'#10,
    FAutoFormat.Process('a :='#10'hello'#10'and world'));

  CheckEquals('a :='#10 + #9'    hello'#10 + #9'and again'#10 +
    #9'and world'#10,
    FAutoFormat.Process('a :='#10'hello'#10'and again'#10'and world'));
end;
...
begin
  CheckEquals('aaaaaaaaaaaaaaaaaaaa := if bbbbbbbbbbbbbbbbbbbbb then'#10 +
    #9#9'ccccccccccccccccccc'#10 + #9'else ddddddddddddddddddddd'#10,
    FAutoFormat.Process
  ...
  CheckEquals('aaaaaaaaaaaaaaaaaaaa := if bbbbbbbbbbbbbbbbbbbbb then'#10 +
    ...
    #9#9'fffffffffffffffffffffffff'#10 + #9'else ggggggggggggggggggggg'#10,
    FAutoFormat.Process
  ...
  CheckEquals('if True then begin'#10 + #9'PrintLn(1)'#10 + 'end else begin'#10
    + #9'PrintLn(2)'#10 + 'end;'#10,
    FAutoFormat.Process
