...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'JSFilterScripts' + PathDelim,
    cFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'JSFilterScriptsFail' + PathDelim,
    cFilter, FTestFailures);

  FMainCompiler := TDelphiWebScript.Create(nil);
  FMainCompiler.OnInclude := DoInclude;

  FJSCompiler := TDelphiWebScript.Create(nil);
  FJSCompiler.Config.CompilerOptions := FJSCompiler.Config.CompilerOptions +
...
// DoInclude
//
procedure TJSFilterTests.DoInclude(const scriptName: string;
...
// DoNeedUnit
//
function TJSFilterTests.DoNeedUnit(const unitName: String;
...
begin
  ...
  FChromium.LoadAndWait
    (src + '<script>console.log(document.body ? document.body.innerText : "!done")</script>',
    'http://localhost', '');
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        output := 'Errors >>>>'#10 + ReplaceStr(prog.Msgs.AsInfo, #13#10, #10) +
          'Result >>>>'#10 + output;
      end;
...
begin
  ...
  FJSFilter.CodeGenOptions :=
    [Low(TdwsCodeGenOption) .. High(TdwsCodeGenOption)];
  try
    TestScripts;
  finally
    FJSCompiler.Config.CompilerOptions := FJSCompiler.Config.CompilerOptions -
