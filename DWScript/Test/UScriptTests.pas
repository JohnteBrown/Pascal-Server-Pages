...
begin
  ...
  CollectFiles(basePath + 'OperatorOverloadFail' + PathDelim, cFilter,
    FFailures);
  ...
  CollectFiles(basePath + 'PropertyExpressionsPass' + PathDelim,
    cFilter, FTests);
  CollectFiles(basePath + 'PropertyExpressionsFail' + PathDelim, cFilter,
    FFailures);
...
// DoInclude
//
procedure TScriptTests.DoInclude(const scriptName: String;
...
{$IFDEF FPC}
      ...
      if Copy(ExtractFileName(FTests[i]), 1, 11) = 'div_by_zero' then
        continue;
{$ENDIF}
...
{$IFDEF FPC}
      ...
      if Copy(ExtractFileName(FTests[i]), 1, 10) = 'for_in_str' then
        continue;
      if Copy(ExtractFileName(FTests[i]), 1, 13) = 'unicode_const' then
        continue;
      if Copy(ExtractFileName(FTests[i]), 1, 19) = 'unicode_identifiers' then
        continue;
{$ENDIF}
      source.LoadFromFile(FTests[i]);
  ...
  try
    ...
    begin
      ...
      try
        ...
        on E: Exception do
        begin
          Check(False, FFailures[i] + ', during compile ' + E.ClassName + ': ' +
            E.Message);
        end;
      end;
      ...
      if FileExists(expectedErrorsFileName) then
      begin
      ...
      try
        ...
        on E: Exception do
        begin
          Check(False, FFailures[i] + ', during cleanup ' + E.ClassName + ': ' +
            E.Message);
        end;
      end;
