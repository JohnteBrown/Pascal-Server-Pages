...
Type
  ...
  TDispItem = Class(TInterfacedObject, IDispatch)
    ...
    Function Invoke(DispId: Integer; Const IID: TGUID; LocaleId: Integer;
      Flags: Word; Var AParams; VarResult, ExcepInfo, ArgErr: Pointer)
      : HResult; StdCall;
...
//
Function TDispItem.GetIDsOfNames(Const IID: TGUID; Names: Pointer;
...
//
Function TDispItem.Invoke(DispId: Integer; const IID: TGUID; LocaleId: Integer;
  Flags: Word; Var AParams; VarResult, ExcepInfo, ArgErr: Pointer): HResult;
Const
  ...
  Begin
    ...
    Begin
      ...
      If TDispParams(AParams).cArgs > MaxDispArgs Then
        Raise Exception.Create('To many params ' +
          IntToStr(TDispParams(AParams).cArgs));
  ...
  Begin
    ...
    For Index := VarArrayLowBound(AArray, 1) To VarArrayHighBound(AArray, 1) Do
      Result := Result + Format('%s    [%d] %s',
        [sLineBreak, Index, VarTypeAsText(TVarData(VarArrayGet(AArray, [Index])
        ).VType)]);
...
Begin
  ...
  Begin
    ...
    S := S + LineBreak[S <> ''] + Format('param index: %d, type: %s (%d)',
      [Index, VarTypeAsText(TVarData(FindVarData(AParams[Index])^).VType),
      TVarData(FindVarData(AParams[Index])^).VType]);
