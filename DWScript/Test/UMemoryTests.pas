...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'Memory' + PathDelim,
    '*.pas', FTests);
...
// DoCreateExternal
//
procedure TMemoryTests.DoCreateExternal(Info: TProgramInfo;
...
//
procedure TMemoryTests.DoCreateBoomExternal(Info: TProgramInfo;
  var ExtObject: TObject);
begin
...
// DoKeepExternalRef
//
procedure TMemoryTests.DoKeepExternalRef(Info: TProgramInfo;
  ExtObject: TObject);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      CheckEquals(0, exec.ObjectCount, FTests[i] + ', leaked ' +
        IntToStr(exec.ObjectCount) + ' script objects');
      CheckEquals(0, FTestObjects.Count, FTests[i] + ', leaked ' +
        IntToStr(FTestObjects.Count) + ' external objects');
...
begin
  ...
  try
    ...
    CheckEquals('TExposedClass', FExternalRef.ClassSym.Name,
      'external ref classname');
    CheckEquals(False, (FExternalRef.externalObject as TTestObject)
      .FAlreadyDestroyed, 'external ref ok');
  finally
    exec.EndProgram;
  end;
  ...
  CheckEquals(False, (FExternalRef.externalObject as TTestObject)
    .FAlreadyDestroyed, 'external not dead just yet');

  exec := nil;

  CheckEquals(1, vTestObjetCount, 'test object count after exec death');
  CheckTrue(FExternalRef.ClassSym = nil,
    'external ref disconnected after death');
  CheckEquals(False, (FExternalRef.externalObject as TTestObject)
    .FAlreadyDestroyed, 'external still not dead');

  FExternalRef := nil;

  CheckEquals(0, vTestObjetCount,
    'test object count after external ref dropped');
end;
