...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'COMConnector' + PathDelim,
    '*.pas', FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'COMConnectorFailure' + PathDelim,
    '*.pas', FFailures);
...
{$IFDEF WIN32}
  ...
  conn.ConnectionString :=
    'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=Data\Db.mdb;Persist Security Info=False';
  conn.Open;
...
{$ELSE}
  ...
  conn := CreateOleObject('ADODB.Connection');
  conn.ConnectionString :=
    'Provider=Microsoft.ACE.OLEDB.12.0;Data Source=Data\Db.mdb;Persist Security Info=False';
  conn.Open;

{$ENDIF}
  conn.Execute
    ('create table test (intCol INT, charCol VARCHAR(50), memoCol MEMO, dateCol DATE)');
  conn.Execute
    ('insert into test values (10, ''ten'', ''value ten'', cdate(''2010-10-10''))');
  conn.Execute
    ('insert into test values (20, ''twenty'', ''value twenty'', cdate(''2020-10-20''))');
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        output := 'Errors >>>>'#13#10 + exec.Msgs.AsInfo + 'Result >>>>'#13#10 +
