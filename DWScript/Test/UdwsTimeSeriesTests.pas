...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'TimeSeriesLib' + PathDelim,
    '*.pas', FTests);
...
const
  cTestDoubles: array [0 .. 10] of Double = (1.0, 1.0, -1.0, 3.5, -1.5, 20.0,
...
begin
  packedData := nil;
  for var i := 0 to High(cTestDoubles) do
  begin
    var
    n := DeltaPackValues(@cTestDoubles[0], i, 10, packedData);
    CheckEquals(i > 0, n > 0, 'length for ' + IntToStr(i));
    DeltaUnpackValues(packedData, i, 0.1, @temp[0]);
    for var k := 0 to i - 1 do
      CheckEquals(cTestDoubles[k], temp[k], 'Pass ' + IntToStr(i) + ' offset ' +
        IntToStr(k));
  end;
