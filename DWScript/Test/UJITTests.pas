...
uses
  ...
  dwsJITFixups, dwsJITx86, dwsJITx86_64, dwsJITx86Intrinsics,
  dwsCompilerContext,
  dwsJSONConnector;
...
type
  ...
  protected
    procedure DoInclude(const scriptName: UnicodeString;
      var scriptSource: UnicodeString);
    function DoNeedUnit(const unitName: UnicodeString;
...
begin
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'InterfacesPass' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'OperatorOverloadPass' +
  ...
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'FunctionsVariant' + PathDelim,
    pasFilter, FTests);
  CollectFiles(ExtractFilePath(ParamStr(0)) + 'FunctionsRTTI' + PathDelim,
    pasFilter, FTests);
...
// DoInclude
//
procedure TJITTests.DoInclude(const scriptName: UnicodeString;
  var scriptSource: UnicodeString);
var
...
// DoNeedUnit
//
function TJITTests.DoNeedUnit(const unitName: UnicodeString;
  var unitSource: UnicodeString): IdwsUnit;
var
  sl: TStringList;
  fName: UnicodeString;
...
// GetExpectedResult
//
function TJITTests.GetExpectedResult(const fileName: UnicodeString)
  : UnicodeString;
var
...
begin
  ...
  try
    ...
    begin
      ...
      try
        jit := {$IFDEF WIN32} TdwsJITx86 {$ENDIF}{$IFDEF WIN64} TdwsJITx86_64
        {$ENDIF}.Create;
        try
        ...
        on E: Exception do
          raise Exception.Create(FTests[i] + ', ' + E.ClassName + ': ' +
            E.Message)
      end;
      ...
      begin
        diagnostic.Add(ExtractFileName(FTests[i]) + ': expected <' +
          expectedResult + '> but got <' + output + '>');
      end;
