...
type
  ...
  IDataContext = interface(IGetSelf)
    ...
    property AsVariant[addr: NativeInt]: Variant read GetAsVariant
      write SetAsVariant; default;

    function AsPData: PData;

    procedure CreateOffset(offset: NativeInt; var result: IDataContext);

    property AsInteger[addr: NativeInt]: Int64 read GetAsInteger
      write SetAsInteger;
    property AsBoolean[addr: NativeInt]: Boolean read GetAsBoolean
      write SetAsBoolean;
    ...
    property AsInterface[addr: NativeInt]: IUnknown read GetAsInterface
      write SetAsInterface;
    ...
    function SameData(addr: NativeInt; const other: IDataContext;
      otherAddr, size: NativeInt): Boolean; overload;
...
{$ENDIF}
  ...
  public
    ...
    procedure CopyData(const destData: TData; destAddr, size: NativeInt);
      overload; inline;
    procedure CopyData(addr: NativeInt; const destData: TData;
      destAddr, size: NativeInt); overload; inline;
    procedure CopyData(addr: NativeInt; const destPVariant: PVariant;
      size: NativeInt); overload; inline;

    procedure WriteData(const src: IDataContext; size: NativeInt);
      overload; inline;
    procedure WriteData(destAddr: NativeInt; const src: IDataContext;
      srcAddr, size: NativeInt); overload; inline;

    procedure MoveData(srcAddr, destAddr, size: NativeInt); inline;

    function SameData(addr: NativeInt; const otherData: IDataContext;
      otherAddr, size: NativeInt): Boolean; overload; inline;
    function SameData(const other: IDataContext): Boolean; overload;

    function IndexOfData(const item: IDataContext;
      fromIndex, toIndex, itemSize: NativeInt): NativeInt;
    function IndexOfValue(const item: Variant; fromIndex, toIndex: NativeInt)
      : NativeInt;
...
procedure DWSCopyData(const data: TData; sourceAddr, destAddr: NativeInt;
  size: NativeInt); overload;

procedure DWSMoveData(const data: TData; sourceAddr, destAddr, size: NativeInt);

function DWSSameData(const data1, data2: TData;
  offset1, offset2, size: NativeInt): Boolean; overload;
...
function DWSHashCode(const data: TData; offset, size: NativeInt)
  : Cardinal; overload;
...
begin
  ...
  if sizeBytes <= cStaticBufferSize then
    ...
    buf := GetMemory(sizeBytes);

  System.Move(data[sourceAddr], buf^, sizeBytes);
  if sourceAddr < destAddr then
    System.Move(data[sourceAddr + size], data[sourceAddr],
      SizeOf(Variant) * (destAddr - sourceAddr))
  else
    System.Move(data[destAddr], data[destAddr + size],
      SizeOf(Variant) * (sourceAddr - destAddr));
...
//
function DWSSameData(const data1, data2: TData;
  offset1, offset2, size: NativeInt): Boolean;
var
  i: NativeInt;
begin
  for i := 0 to size - 1 do
...
begin
  ...
  case p.VType of
    ...
    varInteger, varSingle, varLongWord, varUnknown, varDispatch:
      begin // 32 bits
        partial := (partial xor SimpleIntegerHash(p.VLongWord)) * cFNV_prime;
      end;
    varInt64, varDouble, varCurrency, varDate, varUInt64:
      begin // 64 bits
    ...
    varUString:
      begin
        if p.VUString <> nil then
          partial := (partial xor SimpleStringHash(String(p.VUString))) *
            cFNV_prime
        else
          partial := partial * cFNV_prime;
      end;
{$ENDIF}
    varString:
      begin
        if p.VString <> nil then
          partial := (partial xor SimpleByteHash(p.VString,
            Length(AnsiString(p.VString)))) * cFNV_prime
        else
          partial := partial * cFNV_prime;
      end;
    varOleStr:
      begin
        if p.VOleStr <> nil then
          partial := (partial xor SimpleStringHash(p.VOleStr, Length(p.VOleStr))
            ) * cFNV_prime
        else
          partial := partial * 16777619;
      end;
  else
...
begin
  result := (TVarData(v).VType = varEmpty) or
    ((TVarData(v).VType = varUnknown) and (TVarData(v).VUnknown = nil));
end;
...
// CreateData
//
function TDataContextPool.CreateData(const aData: TData; anAddr: NativeInt)
  : TDataContext;
begin
  result := Pop;
  result.FAddr := anAddr;
...
// CreateOffset
//
function TDataContextPool.CreateOffset(offset: NativeInt; ref: TDataContext)
  : TDataContext;
begin
  result := Pop;
...
begin
  ...
  if p^.VType = varInt64 then
    ...
    result := VariantToInt64(PVariant(p)^);
end;
...
begin
  ...
  if p^.VType = varDouble then
    ...
    result := VariantToFloat(PVariant(p)^);
end;
...
begin
  ...
  if p^.VType = varBoolean then
    ...
    result := VariantToBool(PVariant(p)^);
end;
...
// CreateOffset
//
procedure TDataContext.CreateOffset(offset: NativeInt;
...
begin
  if FPool <> nil then
    ...
    result := CreateData(Self, FAddr + offset);
end;
...
{$IFDEF FPC}
  if p.VType = varString then
    ...
    VariantToString(PVariant(p)^, result);
{$ELSE}
  if p.VType = varUString then
...
// CopyData
//
procedure TDataContext.CopyData(const destData: TData;
  destAddr, size: NativeInt);
begin
...
// CopyData
//
procedure TDataContext.CopyData(addr: NativeInt; const destData: TData;
  destAddr, size: NativeInt);
begin
...
// CopyData
//
procedure TDataContext.CopyData(addr: NativeInt; const destPVariant: PVariant;
  size: NativeInt);
begin
...
// WriteData
//
procedure TDataContext.WriteData(destAddr: NativeInt; const src: IDataContext;
  srcAddr, size: NativeInt);
var
  i: NativeInt;
  pDest: PVariant;
begin
  ...
  for i := srcAddr to srcAddr + size - 1 do
  begin
...
// SameData
//
function TDataContext.SameData(addr: NativeInt; const otherData: IDataContext;
  otherAddr, size: NativeInt): Boolean;
begin
  result := DWSSameData(FData, otherData.AsPData^, FAddr + addr,
    otherData.addr + otherAddr, size);
end;
...
// IndexOfData
//
function TDataContext.IndexOfData(const item: IDataContext;
  fromIndex, toIndex, itemSize: NativeInt): NativeInt;
var
  i: NativeInt;
begin
...
// IndexOfValue
//
function TDataContext.IndexOfValue(const item: Variant;
  fromIndex, toIndex: NativeInt): NativeInt;
var
  i: NativeInt;
  data: PData;
begin
...
// IndexOfString
//
function TDataContext.IndexOfString(const item: String; fromIndex: NativeInt)
  : NativeInt;
var
  i: NativeInt;
  varData: PVarData;
begin
  if fromIndex < DataLength then
  begin
    varData := @AsPData^[fromIndex];
    for i := fromIndex to DataLength - 1 do
    begin
{$IFDEF FPC}
...
// IndexOfInteger
//
function TDataContext.IndexOfInteger(const item: Int64; fromIndex: NativeInt)
  : NativeInt;
var
  i: NativeInt;
  varData: PVarData;
...
// IndexOfFloat
//
function TDataContext.IndexOfFloat(const item: Double; fromIndex: NativeInt)
  : NativeInt;
var
  i: NativeInt;
  varData: PVarData;
...
// Create
//
constructor TRelativeDataContext.Create(const getPData: TGetPDataFunc;
  addr: NativeInt);
begin
  FGetPData := getPData;
  FAddr := addr;
...
// QueryInterface
//
function TRelativeDataContext.QueryInterface(const IID: TGUID; out Obj)
  : HResult;
begin
  if GetInterface(IID, Obj) then
    result := 0
...
// SetAsVariant
//
procedure TRelativeDataContext.SetAsVariant(addr: NativeInt;
  const value: Variant);
begin
...
// SetAsInteger
//
procedure TRelativeDataContext.SetAsInteger(addr: NativeInt;
  const value: Int64);
begin
...
// SetAsBoolean
//
procedure TRelativeDataContext.SetAsBoolean(addr: NativeInt;
  const value: Boolean);
begin
  VarCopySafe(FGetPData^[FAddr + addr], value);
end;
...
// SetAsString
//
procedure TRelativeDataContext.SetAsString(addr: NativeInt;
  const value: String);
begin
  VarCopySafe(FGetPData^[FAddr + addr], value);
end;
...
// SetAsInterface
//
procedure TRelativeDataContext.SetAsInterface(addr: NativeInt;
...
// CreateOffset
//
procedure TRelativeDataContext.CreateOffset(offset: NativeInt;
  var result: IDataContext);
begin
...
// EvalAsVariant
//
procedure TRelativeDataContext.EvalAsVariant(addr: NativeInt;
  var result: Variant);
begin
...
// EvalAsString
//
procedure TRelativeDataContext.EvalAsString(addr: NativeInt;
  var result: String);
begin
  result := FGetPData^[FAddr + addr];
end;
...
// EvalAsInterface
//
procedure TRelativeDataContext.EvalAsInterface(addr: NativeInt;
  var result: IUnknown);
begin
  result := FGetPData^[FAddr + addr];
end;
...
// CopyData
//
procedure TRelativeDataContext.CopyData(const destData: TData;
  destAddr, size: NativeInt);
begin
...
// WriteData
//
procedure TRelativeDataContext.WriteData(const src: IDataContext;
  size: NativeInt);
begin
...
// WriteData
//
procedure TRelativeDataContext.WriteData(destAddr: NativeInt;
  const src: IDataContext; srcAddr, size: NativeInt);
begin
  DWSCopyData(src.AsPData^, src.addr + srcAddr, FGetPData^,
    FAddr + destAddr, size);
end;

// SameData
//
function TRelativeDataContext.SameData(addr: NativeInt;
  const other: IDataContext; otherAddr, size: NativeInt): Boolean;
begin
  if other.GetSelf is TRelativeDataContext then
    otherAddr := TRelativeDataContext(other.GetSelf).FAddr + otherAddr;
  result := DWSSameData(AsPData^, other.AsPData^, FAddr + addr,
    otherAddr, size);
end;
