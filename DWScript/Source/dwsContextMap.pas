...
type
  ...
  private
    ...
    // symbol table associated with the context (begin..end blocks, TProcedures, etc)
  ...
  public
    ...
    function IsPositionInContext(aCol, aLine: Integer; const sourceName: String)
      : Boolean; overload;
    function HasParentSymbolOfClass(SymbolType: TSymbolClass;
    ...
    procedure EnumerateContextsOfSymbol(aParentSymbol: TSymbol;
      const callBack: TdwsSourceContextCallBack);
  ...
  private
    ...
    FCurrentContext: TdwsSourceContext;
    // current context (used when adding and leaving)
  ...
  public
    ...
    procedure OpenContext(const StartPos: TScriptPos; parentSymbol: TSymbol;
      Token: TTokenType);
    { Pop a context off the stack }
    procedure CloseContext(const aEndPos: TScriptPos;
      onlyIfTokenType: TTokenType = ttNone);
    ...
    function FindContext(aCol, aLine: Integer; sourceFile: TSourceFile)
      : TdwsSourceContext; overload;
    function FindContext(aCol, aLine: Integer; const sourceName: String)
      : TdwsSourceContext; overload;
    function FindContext(const ScriptPos: TScriptPos)
      : TdwsSourceContext; overload;
    function FindContextByToken(aToken: TTokenType): TdwsSourceContext;
    procedure EnumerateContextsOfSymbol(aParentSymbol: TSymbol;
      const callBack: TdwsSourceContextCallBack);
...
// Create
//
constructor TdwsSourceContext.Create(aParent: TdwsSourceContext;
  const aStartPos: TScriptPos; aParentSymbol: TSymbol; aToken: TTokenType);
begin
  inherited Create;
...
// FindContext
//
function TdwsSourceContext.FindContext(parentSymbol: TSymbol)
  : TdwsSourceContext;
var
  i: Integer;
begin
  if FParentSymbol = parentSymbol then
    Exit(Self);
...
// FindContextByToken
//
function TdwsSourceContext.FindContextByToken(aToken: TTokenType)
  : TdwsSourceContext;
var
  i: Integer;
begin
...
//
procedure TdwsSourceContext.EnumerateContextsOfSymbol(aParentSymbol: TSymbol;
  const callBack: TdwsSourceContextCallBack);
var
  i: Integer;
begin
...
begin
  ...
  writer.WriteString(UnicodeString(GetEnumName(TypeInfo(TTokenType),
    Ord(Token))));
...
// IsPositionInContext
//
function TdwsSourceContext.IsPositionInContext(aCol, aLine: Integer;
  const sourceName: String): Boolean;
begin
  // check if the position is in the same SourceFile
  if sourceName <> '' then
  begin // if empty, don't check it
    if not FStartPos.IsSourceFile(sourceName) then
    begin
      Result := False;
      Exit;
    end;
  ...
  begin
    ...
    if FStartPos.Line = FEndPos.Line then
      Result := (aLine = FStartPos.Line) and (aCol >= FStartPos.Col) and
        (aCol <= FEndPos.Col)
    else
    begin
      // not a single-line context
      if FEndPos.sourceFile = nil then
      begin
        // unclosed context (compiled with errors)
        Result := (aLine > FStartPos.Line) or
          ((aLine = FStartPos.Line) and (aCol >= FStartPos.Col)); // after start
      end
      else
      begin
        Result := ((aLine = FStartPos.Line) and (aCol >= FStartPos.Col))
        // on top line, inside start
          or ((aLine = FEndPos.Line) and (aCol <= FEndPos.Col));
        // on bottom line, inside end
      end;
    end;
  end;
end;
...
// FindContext
//
function TdwsSourceContextMap.FindContext(aParentSymbol: TSymbol)
  : TdwsSourceContext;
var
...
// FindContextByToken
//
function TdwsSourceContextMap.FindContextByToken(aToken: TTokenType)
  : TdwsSourceContext;
var
  sc: TRefCountedObject;
begin
...
// EnumerateContextsOfSymbol
//
procedure TdwsSourceContextMap.EnumerateContextsOfSymbol(aParentSymbol: TSymbol;
  const callBack: TdwsSourceContextCallBack);
var
  sc: TRefCountedObject;
begin
...
// FindContext
//
function TdwsSourceContextMap.FindContext(aCol, aLine: Integer;
  sourceFile: TSourceFile): TdwsSourceContext;
begin
...
//
function TdwsSourceContextMap.FindContext(aCol, aLine: Integer;
  const sourceName: String): TdwsSourceContext;
var
...
// FindContext
//
function TdwsSourceContextMap.FindContext(const ScriptPos: TScriptPos)
  : TdwsSourceContext;
begin
...
// OpenContext
//
procedure TdwsSourceContextMap.OpenContext(const StartPos: TScriptPos;
  parentSymbol: TSymbol; Token: TTokenType);
var
...
begin
  ...
  newContext := TdwsSourceContext.Create(FCurrentContext, StartPos,
    parentSymbol, Token);
...
// CloseContext
//
procedure TdwsSourceContextMap.CloseContext(const aEndPos: TScriptPos;
  onlyIfTokenType: TTokenType = ttNone);
begin
  if (onlyIfTokenType <> ttNone) and (FCurrentContext.Token <> onlyIfTokenType)
  then
    Exit;
...
begin
  Assert((FCurrentContext = nil) or (FCurrentContext.Token in [ttUNIT,
    ttINTERFACE]));
