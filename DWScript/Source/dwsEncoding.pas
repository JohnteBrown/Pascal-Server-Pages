...
function Base64Encode(data: Pointer; len: Integer;
  const alphabet: TBase64Alphabet; isMime: Boolean = False): String; overload;
function Base64EncodeToBytes(data: Pointer; len: Integer;
  const alphabet: TBase64Alphabet): TBytes;
...
const
  ...
  cBase64: TBase64Alphabet =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
  cBase64URI: TBase64Alphabet =
...
begin
  ...
  for i := Length(Result) to len - 1 do
  begin
...
begin
  ...
  for i := Length(Result) + 1 to Length(data) do
  begin
...
const
  cBase32: array [0 .. 31] of Char = ('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H',
...
function Base32Decode(const data: String): RawByteString;
  ...
  var
    c: Char;
  begin
    for c := #0 to High(vBase32DecodeTable) do
    begin
      case c of
        'A' .. 'Z':
          vBase32DecodeTable[c] := Ord(c) - Ord('A');
        'a' .. 'z':
          vBase32DecodeTable[c] := Ord(c) - Ord('a');
        '2' .. '7':
          vBase32DecodeTable[c] := Ord(c) + (26 - Ord('2'));
        '0':
          vBase32DecodeTable[c] := Ord('O') - Ord('A');
      else
...
begin
  if data = '' then
    exit('');
  if vBase32DecodeTable['z'] = 0 then
    PrepareTable;
  ...
  begin
    ...
    begin
      if pIn[i] = '=' then
        break;
      raise EdwsEncoding.CreateFmt('Invalid character (#%d) in Base32',
        [Ord(pIn[i])]);
    end;
    c := (c shl 5) or d;
    Inc(b, 5);
...
begin
  ...
  for i := 0 to High(vBase64Decode) do
  begin
    case Char(i) of
      #1 .. #32:
        vBase64Decode[i] := -2;
      'A' .. 'Z':
        vBase64Decode[i] := i - Ord('A');
      'a' .. 'z':
        vBase64Decode[i] := i + (26 - Ord('a'));
      '0' .. '9':
        vBase64Decode[i] := i + (52 - Ord('0'));
      '+', '-':
        vBase64Decode[i] := 62;
      '/', '_':
        vBase64Decode[i] := 63;
...
begin
  ...
  begin
    ...
    if isMime then
    begin
      if (lineLength >= 76 - 4) and (i < blocks) then
      begin
        dest[0] := #13;
        dest[1] := #10;
        Inc(dest, 2);
...
function Base64Decode(const data: String): RawByteString;
  ...
  begin
    c := 0;
    while NativeUInt(src) <= NativeUInt(srcLast) do
    begin
...
begin
  ...
  SetLength(Result, outLen);
  decodedPtr := Base64Decode(Pointer(data), Pointer(@data[len - 3]),
    Pointer(Result));
