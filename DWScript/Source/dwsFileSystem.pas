...
type
  ...
  // TdwsFileOpenMode
  //
  TdwsFileOpenMode = (fomReadOnly,
    // opens file for read-only, fails if doesn't exist
    fomReadWrite,
    // opens file for read-write, creates a new one if doesn't exist
    fomCreate, // opens file for read-write, always empty
    fomFastSequentialRead
    // opens file for sequential read-only, return nil if doesn't exist
    );

  TdwsFileSystemTextFormat = (fstfUTF8_with_BOM, fstfUTF8_no_BOM);

  EdwsFileSystemException = class(Exception);

  EdwsFSInvalidFileName = class(EdwsFileSystemException);

  TFileStreamOpenedEvent = procedure(Sender: TObject; const fileName: TFileName;
    const mode: TdwsFileOpenMode) of object;
  ...
  IdwsFileSystem = interface
    ...
    function OpenFileStream(const fileName: TFileName;
      const mode: TdwsFileOpenMode): TStream;
  ...
  IdwsFileSystemRW = interface(IdwsFileSystem)
    ...
    function DeleteDirectory(const directoryName: TFileName;
    ...
    procedure SaveTextFile(const fileName: TFileName; const strData: String;
      fmt: TdwsFileSystemTextFormat);
  end;
  ...
  protected
    ...
    procedure DoFileStreamOpenedEvent(const fileName: TFileName;
      const mode: TdwsFileOpenMode); inline;
  ...
  public
    ...
    function CreateDirectory(const directoryName: TFileName): Boolean;
      virtual; abstract;
    function DeleteDirectory(const directoryName: TFileName; recursive: Boolean)
      : Boolean; virtual; abstract;
    function ForceDirectories(const directoryName: TFileName): Boolean;
      virtual; abstract;
    function OpenFileStream(const fileName: TFileName;
      const mode: TdwsFileOpenMode): TStream; virtual; abstract;
    function ValidateFileName(const fileName: TFileName): TFileName;
      virtual; abstract;
    function FileTimeStamp(const fileName: TFileName): TdwsDateTime;
      virtual; abstract;
    ...
    property OnFileStreamOpened: TFileStreamOpenedEvent read FOnFileStreamOpened
      write FOnFileStreamOpened;
  ...
  public
    ...
    function OpenFileStream(const fileName: TFileName;
      const mode: TdwsFileOpenMode): TStream; override;
...
//
procedure TdwsBaseFileSystem.DoFileStreamOpenedEvent(const fileName: TFileName;
  const mode: TdwsFileOpenMode);
begin
...
// LoadTextFile
//
function TdwsBaseFileSystem.LoadTextFile(const fileName: TFileName)
  : UnicodeString;
begin
  var
...
// SaveTextFile
//
procedure TdwsBaseFileSystem.SaveTextFile(const fileName: TFileName;
  const strData: String; fmt: TdwsFileSystemTextFormat);
const
...
begin
  var
  stream := OpenFileStream(fileName, fomCreate);
  if stream = nil then
...
// DirectoryExists
//
function TdwsNullFileSystem.DirectoryExists(const directoryName
  : TFileName): Boolean;
begin
  Result := False;
end;
...
// CreateDirectory
//
function TdwsNullFileSystem.CreateDirectory(const directoryName
  : TFileName): Boolean;
begin
  Result := False;
end;
...
// DeleteDirectory
//
function TdwsNullFileSystem.DeleteDirectory(const directoryName: TFileName;
  recursive: Boolean): Boolean;
begin
  Result := False;
end;
...
// ForceDirectories
//
function TdwsNullFileSystem.ForceDirectories(const directoryName
  : TFileName): Boolean;
begin
  Result := False;
end;
...
// OpenFileStream
//
function TdwsNullFileSystem.OpenFileStream(const fileName: TFileName;
  const mode: TdwsFileOpenMode): TStream;
begin
  Result := nil;
end;
...
// ValidateFileName
//
function TdwsNullFileSystem.ValidateFileName(const fileName: TFileName)
  : TFileName;
begin
  Result := '';
end;
...
// FileTimeStamp
//
function TdwsNullFileSystem.FileTimeStamp(const fileName: TFileName)
  : TdwsDateTime;
begin
...
// ValidateFileName
//
function TdwsOSFileSystem.ValidateFileName(const fileName: TFileName)
  : TFileName;
begin
  // accept all
...
// DirectoryExists
//
function TdwsOSFileSystem.DirectoryExists(const directoryName
  : TFileName): Boolean;
var
...
// CreateDirectory
//
function TdwsOSFileSystem.CreateDirectory(const directoryName
  : TFileName): Boolean;
var
  validDirectoryName: TFileName;
begin
...
// DeleteDirectory
//
function TdwsOSFileSystem.DeleteDirectory(const directoryName: TFileName;
  recursive: Boolean): Boolean;
var
  validDirectoryName: TFileName;
begin
...
// ForceDirectories
//
function TdwsOSFileSystem.ForceDirectories(const directoryName
  : TFileName): Boolean;
var
  validDirectoryName: TFileName;
begin
...
// FileTimeStamp
//
function TdwsOSFileSystem.FileTimeStamp(const fileName: TFileName)
  : TdwsDateTime;
var
  validFileName: TFileName;
begin
...
// OpenFileStream
//
function TdwsOSFileSystem.OpenFileStream(const fileName: TFileName;
  const mode: TdwsFileOpenMode): TStream;
var
  validFileName: TFileName;
  hFile: THandle;
begin
  ...
  case mode of
    fomReadOnly:
      Result := TFileStream.Create(validFileName, fmOpenRead or
        fmShareDenyWrite);
    fomReadWrite:
...
// ValidateFileName
//
function TdwsRestrictedOSFileSystem.ValidateFileName(const aFileName: TFileName)
  : TFileName;
var
  i: Integer;
...
begin
  ...
  begin
    ...
    begin
      if StrIBeginsWith(Result, FPaths[i]) then
        Exit;
    end;
  end
  else
  begin
    ...
    begin
      ...
      if StrIBeginsWith(Result, path) and System.SysUtils.FileExists(Result)
      then
        Exit;
    end;
  end;
...
// CreateRestrictedFS
//
function TdwsRestrictedFileSystem.CreateRestrictedFS
  : TdwsRestrictedOSFileSystem;
begin
