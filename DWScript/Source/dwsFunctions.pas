...
type
  ...
  public
    procedure InitSymbol(Symbol: TSymbol;
      const msgs: TdwsCompileMessageList); virtual;
  ...
  public
    ...
    procedure Call(exec: TdwsProgramExecution; func: TFuncSymbol); override;
    procedure Execute(info: TProgramInfo; var ExternalObject: TObject);
      virtual; abstract;
    function Specialize(const context: ISpecializationContext): IExecutable;
  end;
  ...
  public
    constructor Create(methKind: TMethodKind; attributes: TMethodAttributes;
      ...
      aVisibility: TdwsVisibility; table: TSymbolTable;
  ...
  public
    constructor Create(methKind: TMethodKind; attributes: TMethodAttributes;
      const methName: String; const methParams: array of String;
      const methType: String; rec: TRecordSymbol; aVisibility: TdwsVisibility;
      table: TSymbolTable; overloaded: Boolean = False);
  end;

  TInternalInitProc = procedure(systemTable: TSystemSymbolTable;
    unitSyms: TUnitMainSymbols; unitTable: TSymbolTable; operators: TOperators);
  TSymbolsRegistrationProc = procedure(systemTable: TSystemSymbolTable;
    unitSyms: TUnitMainSymbols; unitTable: TSymbolTable);
  TOperatorsRegistrationProc = procedure(systemTable: TSystemSymbolTable;
    unitTable: TSymbolTable; operators: TOperators);

  TInternalAbsHandler = function(context: TdwsCompilerContext;
    const aScriptPos: TScriptPos; argExpr: TTypedExpr): TTypedExpr;
  TInternalSqrHandler = function(context: TdwsCompilerContext;
    const aScriptPos: TScriptPos; argExpr: TTypedExpr): TTypedExpr;
  ...
  public
    ...
    procedure InitStaticSymbols(systemTable: TSystemSymbolTable;
      unitSyms: TUnitMainSymbols; operators: TOperators);
...
procedure RegisterInternalProcedure(InternalFunctionClass
  : TInternalFunctionClass; const funcName: String;
  const FuncParams: array of String; const helperName: String = '';
  const flags: TInternalFunctionFlags = []);
...
function ConvertFuncParams(const FuncParams: array of String): TParamArray;
  ...
  begin
    ...
    if v = 'Unassigned' then
      ...
      paramRec.DefaultValue.AsString[0] := v;
...
// CompileTimeCheck
//
procedure TEmptyFunc.CompileTimeCheck(context: TdwsCompilerContext;
  expr: TFuncExprBase);
begin
  // nothing
end;
...
// Specialize
//
function TEmptyFunc.Specialize(const context: ISpecializationContext)
  : IExecutable;
begin
...
// CompileTimeCheck
//
procedure TFunctionPrototype.CompileTimeCheck(context: TdwsCompilerContext;
  expr: TFuncExprBase);
begin
  // nothing yet
end;
...
constructor TInternalFunction.Create(table: TSymbolTable;
  ...
  const flags: TInternalFunctionFlags; compositeSymbol: TCompositeTypeSymbol;
...
// Specialize
//
function TInternalFunction.Specialize(const context: ISpecializationContext)
  : IExecutable;
begin
  context.AddCompilerError
    ('Specialization of TInternalFunction not yet supported');
end;

// Create
//
constructor TInternalFunction.Create(table: TSymbolTable;
  const funcName: String; const params: array of String; const funcType: String;
  const flags: TInternalFunctionFlags = [];
  compositeSymbol: TCompositeTypeSymbol = nil; const helperName: String = '');
begin
...
// Create
//
constructor TInternalBaseMethod.Create(methKind: TMethodKind;
  attributes: TMethodAttributes; const methName: String;
  const methParams: array of String; const methType: String;
  cls: TCompositeTypeSymbol; aVisibility: TdwsVisibility; table: TSymbolTable;
  overloaded: Boolean = False);
var
  sym: TMethodSymbol;
  params: TParamArray;
...
// Specialize
//
function TInternalBaseMethod.Specialize(const context: ISpecializationContext)
  : IExecutable;
begin
  context.AddCompilerError
    ('Specialization of TInternalBaseMethod not yet supported');
end;
...
// Create
//
constructor TInternalRecordMethod.Create(methKind: TMethodKind;
  attributes: TMethodAttributes; const methName: String;
  const methParams: array of String; const methType: String; rec: TRecordSymbol;
  aVisibility: TdwsVisibility; table: TSymbolTable;
  overloaded: Boolean = False);
var
  sym: TMethodSymbol;
...
// Call
//
procedure TAnonymousFunction.Call(exec: TdwsProgramExecution;
  func: TFuncSymbol);
var
  info: TProgramInfo;
begin
...
// Specialize
//
function TAnonymousFunction.Specialize(const context: ISpecializationContext)
  : IExecutable;
begin
  context.AddCompilerError
    ('Specialization of TAnonymousFunction not yet supported');
end;
...
// Specialize
//
function TAnonymousMethod.Specialize(const context: ISpecializationContext)
  : IExecutable;
begin
  context.AddCompilerError
    ('Specialization of TAnonymousMethod not yet supported');
end;
...
//
procedure TInternalUnit.AddSymbolsRegistrationProc
  (proc: TSymbolsRegistrationProc);
var
  n: Integer;
begin
...
//
procedure TInternalUnit.AddOperatorsRegistrationProc
  (proc: TOperatorsRegistrationProc);
var
  n: Integer;
begin
...
// InitStaticSymbols
//
procedure TInternalUnit.InitStaticSymbols(systemTable: TSystemSymbolTable;
  unitSyms: TUnitMainSymbols; operators: TOperators);
begin
  if FStaticTable = nil then
  begin
...
// GetUnitTable
//
function TInternalUnit.GetUnitTable(systemTable: TSystemSymbolTable;
...
// InitUnitTable
//
procedure TInternalUnit.InitUnitTable(systemTable: TSystemSymbolTable;
  unitSyms: TUnitMainSymbols; unitTable: TSymbolTable);
var
  i: Integer;
  rif: PRegisteredInternalFunction;
begin
  ...
  begin
    rif := PRegisteredInternalFunction(FRegisteredInternalFunctions[i]);
    try
      rif.InternalFunctionClass.Create(unitTable, rif^.funcName,
        rif^.FuncParams, rif^.funcType, rif^.flags, nil, rif^.helperName,
        rif^.deprecatedMsg);
    except
      on e: Exception do
...
function TInternalUnit.QueryInterface({$IFDEF FPC}constref{$ELSE}const
...
begin
  ...
  unitSyms.Find(SYS_SYSTEM).ReferenceInSymbolTable
    (FSymbol.InterfaceTable, True);
  unitSyms.Find(SYS_INTERNAL).ReferenceInSymbolTable
    (FSymbol.InterfaceTable, True);
...
// GetUnitTable
//
function TSourceUnit.GetUnitTable(systemTable: TSystemSymbolTable;
  unitSyms: TUnitMainSymbols; operators: TOperators; rootTable: TSymbolTable)
  : TUnitSymbolTable;
begin
