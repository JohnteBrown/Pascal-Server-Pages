...
type
  ...
  public
    ...
    class function Evaluate(exec: IdwsProgramExecution;
      const anExpression: String; options: TdwsEvaluateOptions = [];
...
begin
  ...
  try
    ...
    begin
      contextProgram :=
        TdwsProgram((exec.ExecutionObject as TdwsProgramExecution).CurrentProg);
    ...
    try
      ...
      if Assigned(scriptPos) and not scriptPos.IsMainModule then
      begin
        unitMainSymbol := contextProgram.UnitMains.Find(scriptPos.SourceName);
        if unitMainSymbol <> nil then
        begin
          attachedDependencies := Copy(unitMainSymbol.Dependencies);
          for i := 0 to High(attachedDependencies) do
          begin
            if contextProgram.Table.IndexOfParent
              (attachedDependencies[i].Table) >= 0 then
              attachedDependencies[i] := nil
            else
              contextProgram.Table.AddParent(attachedDependencies[i].Table);
          end;
        end;
      end;
      try
        ...
        try
          ...
          compiler.AttachTokenizer(compiler.TokenizerRules.CreateTokenizer
            (compiler.Msgs, nil));
          try
            ...
            try
              ...
              begin
                sourceContext := compiler.SourceContextMap.FindContext
                  (scriptPos^);
                while sourceContext <> nil do
                begin
                  if sourceContext.LocalTable <> nil then
                  begin
                    compiler.CurrentProg.EnterSubTable
                      (sourceContext.LocalTable);
                    Break;
                  end;
            ...
            if compiler.Msgs.HasErrors and
              (compiler.Msgs.Count > previousMsgCount) then
            begin
              gotError := True;
              messageString := compiler.Msgs[previousMsgCount].AsInfo;
              for i := previousMsgCount + 1 to compiler.Msgs.Count - 1 do
              begin
                messageString := messageString + #13#10 + compiler.Msgs
                  [i].AsInfo;
              end;
              expr := TConstStringExpr.Create(cNullPos,
                contextProgram.Root.CompilerContext.TypString, messageString);
            end;
...
begin
  Result := (FContextProcedure = (FExecution.ExecutionObject as
    TdwsProgramExecution).CurrentProg);
end;
