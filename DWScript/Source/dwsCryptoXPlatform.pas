...
begin
  ...
  Pointer(buf) := InterlockedCompareExchangePointer
    (Pointer(vProcessUniqueRandom), Pointer(buf), nil);
end;
...
begin
  SetLength(Result, size);
  for i := 0 to size - 1 do
    PChar(Pointer(Result))[i] :=
      Char(cBase64Chars[((digest[i] * 62) shr 8) + 1]);
end;
