...
type
  ...
  public
    constructor Create(context: TdwsCompilerContext;
      const scriptPos: TScriptPos; aBase: TTypedExpr; aItem: TTypedExpr;
      aFromIndex: TTypedExpr); reintroduce; virtual;
    destructor Destroy; override;

    class function ArrayIndexOfExprClass(arraySym: TArraySymbol)
      : TArrayIndexOfExprClass; static;
  ...
  protected
    function DoEval(exec: TdwsExecution; const base: IScriptDynArray): Integer;
      virtual; abstract;
  ...
  protected
    function DoEval(exec: TdwsExecution; const base: IScriptDynArray)
      : Integer; override;
  end;
...
// Create
//
constructor TArrayIndexOfExpr.Create(context: TdwsCompilerContext;
  const scriptPos: TScriptPos; aBase: TTypedExpr; aItem: TTypedExpr;
  aFromIndex: TTypedExpr);
begin
...
//
class function TArrayIndexOfExpr.ArrayIndexOfExprClass(arraySym: TArraySymbol)
  : TArrayIndexOfExprClass;
var
  elemTyp: TTypeSymbol;
  elemTypClass: TClass;
begin
  ...
  begin
    ...
    begin
      ...
      if elemTypClass = TBaseIntegerSymbol then
        ...
        Result := TDynamicArrayIndexOfVariantExpr;
    end;
  end
  else
    Result := TStaticArrayIndexOfExpr;
end;

// EvalAsVariant
//
procedure TArrayIndexOfExpr.EvalAsVariant(exec: TdwsExecution;
  var Result: Variant);
begin
...
// DoEval
//
function TDynamicArrayIndexOfDataExpr.DoEval(exec: TdwsExecution;
  const base: IScriptDynArray): Integer;
var
...
begin
  ...
  Result := dyn.IndexOfData(TDataExpr(FItemExpr).DataPtr[exec], fromIndex,
    dyn.ArrayLength - 1, FItemExpr.Typ.Size)
end;
...
// DoEval
//
function TDynamicArrayIndexOfVariantExpr.DoEval(exec: TdwsExecution;
  const base: IScriptDynArray): Integer;
var
  fromIndex: Integer;
  v: Variant;
...
// DoEval
//
function TDynamicArrayIndexOfFuncPtrExpr.DoEval(exec: TdwsExecution;
  const base: IScriptDynArray): Integer;
var
  fromIndex: Integer;
  v: Variant;
...
// DoEval
//
function TDynamicArrayIndexOfIntegerExpr.DoEval(exec: TdwsExecution;
  const base: IScriptDynArray): Integer;
var
  fromIndex: Integer;
begin
...
// DoEval
//
function TDynamicArrayIndexOfStringExpr.DoEval(exec: TdwsExecution;
  const base: IScriptDynArray): Integer;
var
  fromIndex: Integer;
  v: String;
...
// DoEval
//
function TDynamicArrayIndexOfFloatExpr.DoEval(exec: TdwsExecution;
  const base: IScriptDynArray): Integer;
var
  fromIndex: Integer;
  v: Double;
...
begin
  ...
  if FItemExpr.Typ.Size = 1 then
  begin
    FItemExpr.EvalAsVariant(exec, value);
    Result := dc.IndexOfValue(value, fromIndex - arrayTyp.LowBound,
      arrayTyp.ElementCount - 1);
  end
  else
  begin
    Result := TDataContext(arrayDC.GetSelf)
      .IndexOfData(TDataExpr(FItemExpr).DataPtr[exec],
...
// Create
//
constructor TArrayRemoveExpr.Create(context: TdwsCompilerContext;
  const scriptPos: TScriptPos; aBase: TTypedExpr; aItem: TTypedExpr;
  aFromIndex: TTypedExpr);
var
...
begin
  inherited Create(context, scriptPos, aBase);
  Typ := context.TypInteger;
  indexOfClass := TArrayIndexOfExpr.ArrayIndexOfExprClass
    (aBase.Typ as TDynamicArraySymbol);
  FIndexOf := indexOfClass.Create(context, scriptPos, aBase, aItem, aFromIndex)
    as TDynamicArrayIndexOfExpr;
  aBase.IncRefCount;
end;
...
// EvalAsVariant
//
procedure TArrayRemoveExpr.EvalAsVariant(exec: TdwsExecution;
  var Result: Variant);
begin
  VarCopySafe(Result, EvalAsInteger(exec));
end;
