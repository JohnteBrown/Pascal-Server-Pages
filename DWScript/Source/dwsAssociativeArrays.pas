...
uses
  ...
  dwsSymbols, dwsDataContext, dwsExprs;
...
type
  ...
  protected
    ...
    function LinearValueFind(const keyValue: Variant;
  ...
  public
    class function CreateNew(keyTyp, elemTyp: TTypeSymbol)
      : TScriptAssociativeArray; static;

    function ScriptTypeName: String; override;

    procedure GetDataPtr(exec: TdwsExecution; index: TTypedExpr;
      var result: IDataContext);
    procedure GetDataAsVariant(exec: TdwsExecution; const keyValue: Variant;
      var result: Variant); overload;
    function GetDataAsBoolean(exec: TdwsExecution; index: TTypedExpr)
      : Boolean; overload;
    function GetDataAsBoolean(exec: TdwsExecution; const keyValue: Variant)
      : Boolean; overload;
    function GetDataAsInteger(exec: TdwsExecution; index: TTypedExpr)
      : Int64; overload;
    function GetDataAsInteger(exec: TdwsExecution; const keyValue: Variant)
      : Int64; overload;
    procedure GetDataAsString(exec: TdwsExecution; index: TTypedExpr;
      var result: String); overload;
    procedure GetDataAsString(exec: TdwsExecution; const keyValue: Variant;
      var result: String); overload;

    procedure ReplaceValue(exec: TdwsExecution;
      index, value: TTypedExpr); overload;
    procedure ReplaceValue(exec: TdwsExecution;
      const key, value: Variant); overload;
    ...
    function ReadBucket(index: Integer; var key: TData;
      var value: IDataContext): Boolean;
...
procedure CreateNewAssociativeArray(keyTyp, elemTyp: TTypeSymbol;
...
procedure CreateNewAssociativeArray(keyTyp, elemTyp: TTypeSymbol;
  var result: IScriptAssociativeArray);
begin
  result := TScriptAssociativeArray.CreateNew(keyTyp, elemTyp)
    as IScriptAssociativeArray;
end;
...
//
procedure TScriptAssociativeArray_InitData(typ: TTypeSymbol;
  const resultDC: IDataContext; offset: NativeInt);
var
  a: TAssociativeArraySymbol;
begin
  a := (typ as TAssociativeArraySymbol);
  resultDC.AsInterface[offset] := IScriptAssociativeArray
    (TScriptAssociativeArray.CreateNew(a.KeyType, a.typ));
end;

// CreateNew
//
class function TScriptAssociativeArray.CreateNew(keyTyp, elemTyp: TTypeSymbol)
  : TScriptAssociativeArray;
var
  size: Integer;
  ct: TClass;
begin
  ...
  begin
    ...
    result.FCreateKeyOnAccess := (ct = TDynamicArraySymbol) or
      (ct = TAssociativeArraySymbol) or (size > 1)
  end
  else
    size := 0;
...
begin
  ...
  n := FCapacity - 1;
  for i := 0 to High(oldHashCodes) do
  begin
    if oldHashCodes[i] = 0 then
      continue;
    ...
    DWSCopyData(oldData, i * FElementSize, AsPData^, j * FElementSize,
      FElementSize);
  end;
end;
...
// LinearFind
//
function TScriptAssociativeArray.LinearFind(const item: IDataContext;
  var index: Integer): Boolean;
begin
  repeat
...
// LinearValueFind
//
function TScriptAssociativeArray.LinearValueFind(const keyValue: Variant;
  var index: Integer): Boolean;
begin
  repeat
    if FHashCodes[index] = 0 then
...
// ReplaceValue
//
procedure TScriptAssociativeArray.ReplaceValue(exec: TdwsExecution;
  index, value: TTypedExpr);

  procedure WriteDataExpr(index: Integer);
  begin
    WriteData(index * FElementSize, (value as TDataExpr).DataPtr[exec], 0,
      FElementSize)
  end;
...
begin
  ...
  if FElementSize > 1 then
    ...
    value.EvalAsVariant(exec, DirectData[Addr + i]);
end;
...
begin
  ...
  i := (hashCode and (FCapacity - 1));
  if not LinearValueFind(key, i) then
  begin
    FHashCodes[i] := hashCode;
    FKeys[i] := key;
    Inc(FCount);
...
// GetDataPtr
//
procedure TScriptAssociativeArray.GetDataPtr(exec: TdwsExecution;
  index: TTypedExpr; var result: IDataContext);
var
  i: Integer;
  hashCode: Cardinal;
  key: IDataContext;
begin
  ...
  if (FCount > 0) or FCreateKeyOnAccess then
  begin
    IndexExprToKeyAndHashCode(exec, index, key, hashCode);
    i := (hashCode and (FCapacity - 1));
    if LinearFind(key, i) then
    begin
...
// GetDataAsVariant
//
procedure TScriptAssociativeArray.GetDataAsVariant(exec: TdwsExecution;
  const keyValue: Variant; var result: Variant);
var
  hashCode: Cardinal;
  i: Integer;
  dc: IDataContext;
begin
  ...
  begin
    ...
    i := (hashCode and (FCapacity - 1));
    if LinearValueFind(keyValue, i) then
    begin
...
// GetDataAsBoolean
//
function TScriptAssociativeArray.GetDataAsBoolean(exec: TdwsExecution;
  index: TTypedExpr): Boolean;
var
  i: Integer;
  hashCode: Cardinal;
  key: IDataContext;
...
//
function TScriptAssociativeArray.GetDataAsBoolean(exec: TdwsExecution;
  const keyValue: Variant): Boolean;
var
  hashCode: Cardinal;
  i: Integer;
begin
...
// GetDataAsInteger
//
function TScriptAssociativeArray.GetDataAsInteger(exec: TdwsExecution;
  index: TTypedExpr): Int64;
var
  i: Integer;
  hashCode: Cardinal;
  key: IDataContext;
...
// GetDataAsString
//
procedure TScriptAssociativeArray.GetDataAsString(exec: TdwsExecution;
  index: TTypedExpr; var result: String);
var
  i: Integer;
  hashCode: Cardinal;
  key: IDataContext;
...
//
procedure TScriptAssociativeArray.GetDataAsString(exec: TdwsExecution;
  const keyValue: Variant; var result: String);
var
  hashCode: Cardinal;
  i: Integer;
begin
...
// ContainsKey
//
function TScriptAssociativeArray.ContainsKey(exec: TdwsExecution;
  index: TTypedExpr): Boolean;
var
  i: Integer;
  hashCode: Cardinal;
...
// Delete
//
function TScriptAssociativeArray.Delete(exec: TdwsExecution;
  index: TTypedExpr): Boolean;
var
  i, k, gap: Integer;
  hashCode: Cardinal;
...
begin
  ...
  repeat
    ...
    if ((gap >= k) or (k > i)) and ((i >= gap) or (k <= gap)) and
      ((i >= gap) or (k > i)) then
    begin
...
// ReadBucket
//
function TScriptAssociativeArray.ReadBucket(index: Integer; var key: TData;
  var value: IDataContext): Boolean;
begin
  if Cardinal(index) >= Cardinal(FCapacity) then
    Exit(False);
  if FHashCodes[index] = 0 then
    Exit(False);
...
TAssociativeArraySymbol.SetInitAssociativeArrayProc
  (TScriptAssociativeArray_InitData);
