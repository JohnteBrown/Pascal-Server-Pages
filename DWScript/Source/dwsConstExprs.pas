...
type
  ...
  public
    ...
    constructor CreateValue(const scriptPos: TScriptPos; aTyp: TTypeSymbol;
      const value: Variant);
    constructor CreateData(const scriptPos: TScriptPos; aTyp: TTypeSymbol;
      const initData: IDataContext);
    ...
    class function CreateTyped(context: TdwsCompilerContext; Typ: TTypeSymbol;
      const initData: IDataContext): TConstExpr; overload; static;
    class function CreateTyped(context: TdwsCompilerContext; Typ: TTypeSymbol;
      constSymbol: TConstSymbol): TConstExpr; overload; static;
  end;
  ...
  public
    constructor Create(const scriptPos: TScriptPos; Typ: TTypeSymbol;
      const aValue: Int64);
  ...
  public
    constructor Create(context: TdwsCompilerContext;
      const scriptPos: TScriptPos; symbol: TConstSymbol);
  ...
  public
    ...
    procedure AddElementExpr(const scriptPos: TScriptPos;
      context: TdwsCompilerContext; ElementExpr: TTypedExpr);
    procedure AddElementRange(context: TdwsCompilerContext;
      const range1, range2: Int64; Typ: TTypeSymbol);
    ...
    procedure EvalAsVariant(exec: TdwsExecution; var result: Variant); override;
    procedure EvalToDataContext(exec: TdwsExecution; const destDC: IDataContext;
      offset: Integer);
...
// CreateValue
//
constructor TConstExpr.CreateValue(const scriptPos: TScriptPos;
  aTyp: TTypeSymbol; const value: Variant);
begin
...
// CreateDC
//
constructor TConstExpr.CreateData(const scriptPos: TScriptPos;
  aTyp: TTypeSymbol; const initData: IDataContext);
begin
  Create(scriptPos, aTyp);
...
// CreateNull
//
constructor TConstExpr.CreateNull(const scriptPos: TScriptPos;
  aTyp: TTypeSymbol);
var
  i: Integer;
begin
...
// EvalAsScriptObj
//
procedure TConstExpr.EvalAsScriptObj(exec: TdwsExecution;
...
//
procedure TConstExpr.EvalAsScriptObjInterface(exec: TdwsExecution;
...
// SpecializeDataExpr
//
function TConstExpr.SpecializeDataExpr(const context: ISpecializationContext)
  : TDataExpr;
begin
...
// CreateTyped
//
class function TConstExpr.CreateTyped(context: TdwsCompilerContext;
  Typ: TTypeSymbol; const initData: IDataContext): TConstExpr;
begin
  case Typ.Size of
    0:
      result := TConstExpr.CreateNull(cNullPos, Typ);
    1:
      result := (context.CreateConstExpr(Typ, initData.AsVariant[0])
        as TConstExpr);
  else
...
// Create
//
constructor TConstNilExpr.Create(const scriptPos: TScriptPos;
  aTyp: TTypeSymbol);
begin
  inherited Create(scriptPos, aTyp);
...
// EvalAsScriptObj
//
procedure TConstNilExpr.EvalAsScriptObj(exec: TdwsExecution;
  var result: IScriptObj);
begin
  result := nil;
end;
...
// EvalAsScriptObjInterface
//
procedure TConstNilExpr.EvalAsScriptObjInterface(exec: TdwsExecution;
  var result: IScriptObjInterface);
begin
  result := nil;
end;
...
// Create
//
constructor TConstBooleanExpr.Create(const scriptPos: TScriptPos;
  aTyp: TTypeSymbol; const aValue: Boolean);
begin
  inherited Create(scriptPos, aTyp);
  FValue := aValue;
...
// Create
//
constructor TConstIntExpr.Create(const scriptPos: TScriptPos; Typ: TTypeSymbol;
  const aValue: Int64);
begin
...
// Create
//
constructor TConstFloatExpr.Create(const scriptPos: TScriptPos;
  Typ: TTypeSymbol; const aValue: Double);
begin
  inherited Create(scriptPos, Typ);
  FTyp := Typ;
  FValue := aValue;
...
// Create
//
constructor TConstStringExpr.Create(const scriptPos: TScriptPos;
  Typ: TTypeSymbol; const aValue: String);
begin
  inherited Create(scriptPos, Typ);
  FTyp := Typ;
  FValue := aValue;
...
// EvalAsString
//
procedure TConstStringExpr.EvalAsString(exec: TdwsExecution;
  var result: String);
{$IFNDEF WIN32_ASM}
begin
  result := FValue;
...
// Create
//
constructor TConstArrayExpr.Create(context: TdwsCompilerContext;
  const scriptPos: TScriptPos; symbol: TConstSymbol);
begin
...
// Create
//
constructor TArrayConstantExpr.Create(context: TdwsCompilerContext;
  const aScriptPos: TScriptPos);
var
  sas: TStaticArraySymbol;
begin
  sas := TStaticArraySymbol.Create('', context.TypNil,
    context.TypInteger, 0, -1);
...
// AddElementExpr
//
procedure TArrayConstantExpr.AddElementExpr(const scriptPos: TScriptPos;
  context: TdwsCompilerContext; ElementExpr: TTypedExpr);
var
...
begin
  ...
  if ElementExpr.Typ = nil then
    context.Msgs.AddCompilerStopFmt(scriptPos, CPE_IncompatibleTypes,
      [SYS_VOID, Typ.Typ.Caption]);

  arraySymbol := (FTyp as TStaticArraySymbol);
  if arraySymbol.Typ <> context.TypVariant then
  begin
    if arraySymbol.Typ = context.TypNil then
      arraySymbol.Typ := ElementExpr.Typ
    else if arraySymbol.Typ <> ElementExpr.Typ then
    begin
      if (arraySymbol.Typ = context.TypInteger) and
        (ElementExpr.Typ = context.TypFloat) then
        arraySymbol.Typ := context.TypFloat
      else if ElementExpr.Typ.Size = 1 then
      begin
        if not((arraySymbol.Typ = context.TypFloat) and
          (ElementExpr.Typ = context.TypInteger)) then
          arraySymbol.Typ := context.TypVariant;
      end
      else if not ElementExpr.Typ.IsCompatible(Typ.Typ) then
      begin
        context.Msgs.AddCompilerStopFmt(scriptPos, CPE_IncompatibleTypes,
...
// AddElementRange
//
procedure TArrayConstantExpr.AddElementRange(context: TdwsCompilerContext;
  const range1, range2: Int64; Typ: TTypeSymbol);
var
  i: Int64;
  d: Integer;
...
// Prepare
//
procedure TArrayConstantExpr.Prepare(context: TdwsCompilerContext;
  elementTyp: TTypeSymbol);
var
  elemExpr: TRefCountedObject;
begin
  if (elementTyp <> nil) and (FTyp.Typ <> elementTyp) then
  begin
    if (elementTyp.UnAliasedTypeIs(TBaseFloatSymbol) and
      FTyp.Typ.UnAliasedTypeIs(TBaseIntegerSymbol)) then
    begin
      ElementsFromIntegerToFloat(context);
    end
    else if elementTyp.IsCompatible(FTyp.Typ) then
    begin
...
// GetDataPtr
//
procedure TArrayConstantExpr.GetDataPtr(exec: TdwsExecution;
...
// EvalAsVariant
//
procedure TArrayConstantExpr.EvalAsVariant(exec: TdwsExecution;
  var result: Variant);
begin
  ...
  begin
    ...
    Elements[0].EvalAsVariant(exec, result);
end;

// EvalToDataContext
//
procedure TArrayConstantExpr.EvalToDataContext(exec: TdwsExecution;
  const destDC: IDataContext; offset: Integer);
var
  p, s: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      destDC.WriteData(p, (expr as TDataExpr).DataPtr[exec], 0, s);
    Inc(p, s);
  end;
end;
...
// EvalAsVarRecArray
//
function TArrayConstantExpr.EvalAsVarRecArray(exec: TdwsExecution)
...
begin
  ...
  for element in FElementExprs do
  begin
...
// Optimize
//
function TArrayConstantExpr.Optimize(context: TdwsCompilerContext)
  : TProgramExpr;
begin
  result := Self;
  for var i := 0 to FElementExprs.Count - 1 do
  begin
    var
    expr := Elements[i];
...
begin
  if Typ.Typ = nil then
    context.Msgs.AddCompilerErrorFmt(scriptPos, CPE_InvalidConstType,
      [SYS_VOID])
  else if FElementExprs.Count > 0 then
  begin
    elemTyp := Typ.Typ;
    ...
    begin
      expr := Elements[x];
      if not elemTyp.IsCompatible(expr.Typ) then
      begin
        if elemTyp.IsOfType(context.TypInteger) and
          expr.Typ.IsOfType(context.TypFloat) then
          elemTyp := context.TypFloat
        else if elemTyp.IsOfType(context.TypFloat) and
          expr.Typ.IsOfType(context.TypInteger) then
          // handled below
        else if expr.Typ.IsCompatible(elemTyp) then
          elemTyp := expr.Typ
        else if (expr.Typ is TStructuredTypeSymbol) and
          (elemTyp is TStructuredTypeSymbol) then
        begin
          repeat
        ...
        else if (expr.Typ is TStructuredTypeMetaSymbol) and
          (elemTyp is TStructuredTypeMetaSymbol) then
        begin
          repeat
        ...
        else if context.TypVariant.IsCompatible(expr.Typ) and
          context.TypVariant.IsCompatible(elemTyp) then
          elemTyp := context.TypVariant
        else
        begin
          context.Msgs.AddCompilerErrorFmt(scriptPos,
            CPE_AssignIncompatibleTypes, [expr.Typ.Caption, elemTyp.Caption]);
          Exit;
        end;
      end;
    end;
...
//
procedure TArrayConstantExpr.ElementsFromIntegerToFloat
  (context: TdwsCompilerContext);
begin
  for var x := 0 to FElementExprs.Count - 1 do
  begin
    var
    expr := Elements[x];
    if expr.Typ.IsOfType(context.TypInteger) then
    begin
