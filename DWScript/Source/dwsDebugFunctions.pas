...
// RegisterComplexType
//
procedure RegisterDebugTypes(systemTable: TSystemSymbolTable;
  unitSyms: TUnitMainSymbols; unitTable: TSymbolTable);
var
  typLocation: TRecordSymbol;
begin
  if systemTable.FindLocal(SYS_TSOURCECODELOCATION) <> nil then
    exit;

  typLocation := TRecordSymbol.Create(SYS_TSOURCECODELOCATION, nil);
  typLocation.AddField(TFieldSymbol.Create('Name', systemTable.TypString,
    cvPublic));
  typLocation.AddField(TFieldSymbol.Create('File', systemTable.TypString,
    cvPublic));
  typLocation.AddField(TFieldSymbol.Create('Line', systemTable.TypInteger,
    cvPublic));
...
begin
  ...
  if exec.Debugger <> nil then
    ...
    OutputDebugString(args.AsString[0]);
end;
...
// DoEval
//
procedure TCurrentSourceCodeLocation.DoEval(const args: TExprBaseListExec;
...
// DoEval
//
procedure TCallerSourceCodeLocation.DoEval(const args: TExprBaseListExec;
  var result: IDataContext);
var
  expr: TExprBase;
  sp: TScriptPos;
...
RegisterInternalProcedure(TOutputDebugStringFunc, 'OutputDebugString',
  ['m', SYS_STRING]);

RegisterInternalFunction(TCurrentSourceCodeLocation,
  'CurrentSourceCodeLocation', [], SYS_TSOURCECODELOCATION, []);
RegisterInternalFunction(TCallerSourceCodeLocation, 'CallerSourceCodeLocation',
  [], SYS_TSOURCECODELOCATION, []);
