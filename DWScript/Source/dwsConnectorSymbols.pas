...
type
  ...
  IConnectorType = interface
    ...
    function HasMethod(const MethodName: String;
    ...
    function HasIndex(const PropName: String;
      const params: TConnectorParamArray; var TypSym: TTypeSymbol;
  ...
  public
    ...
    function SpecializeConnector(table: TSymbolTable; const qualifier: String)
      : TConnectorSymbol; virtual;
    ...
    function CreateConvExpr(context: TdwsCompilerContext;
      const aScriptPos: TScriptPos; expr: TTypedExpr): TTypedExpr; virtual;

    property connectorType: IConnectorType read FConnectorType
      write FConnectorType;
  end;

  TConnectorFastMember = class(TInterfacedSelfObject, IConnectorFastMember)
  protected
    procedure FastRead(const exec: TdwsExecution; const base: TExprBase;
      var result: Variant); virtual;
    procedure FastWrite(const exec: TdwsExecution;
      const base, value: TExprBase); virtual;
    function FastReadBoolean(const exec: TdwsExecution; const base: TExprBase)
      : Boolean; virtual;
    function FastReadFloat(const exec: TdwsExecution;
      const base: TExprBase): Double;
  end;
...
// Create
//
constructor TConnectorSymbol.Create(const name: String;
  const connectorType: IConnectorType);
begin
...
begin
  result := (inherited DoIsOfType(TypSym)) or (TypSym is TBaseVariantSymbol);
end;

// SpecializeConnector
//
function TConnectorSymbol.SpecializeConnector(table: TSymbolTable;
  const qualifier: String): TConnectorSymbol;
begin
  result := Self;
end;
...
// CreateAssignExpr
//
function TConnectorSymbol.CreateAssignExpr(context: TdwsCompilerContext;
  const aScriptPos: TScriptPos; left: TDataExpr; right: TTypedExpr)
  : TProgramExpr;
begin
...
// CreateConvExpr
//
function TConnectorSymbol.CreateConvExpr(context: TdwsCompilerContext;
  const aScriptPos: TScriptPos; expr: TTypedExpr): TTypedExpr;
var
  exprTyp: TTypeSymbol;
begin
  result := expr;
  exprTyp := expr.Typ.UnAliasedType;
  if exprTyp <> Self then
  begin
...
// FastRead
//
procedure TConnectorFastMember.FastRead(const exec: TdwsExecution;
...
// FastWrite
//
procedure TConnectorFastMember.FastWrite(const exec: TdwsExecution;
  const base, value: TExprBase);
begin
  raise Exception.Create('FastRead not supported by ' + ClassName);
end;
...
// FastReadBoolean
//
function TConnectorFastMember.FastReadBoolean(const exec: TdwsExecution;
  const base: TExprBase): Boolean;
var
  v: Variant;
begin
...
// FastReadFloat
//
function TConnectorFastMember.FastReadFloat(const exec: TdwsExecution;
  const base: TExprBase): Double;
var
  v: Variant;
begin
